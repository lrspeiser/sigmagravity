...
\noindent\textit{Manuscript ID 1866133; 4,115 words; 4 figures; 2 tables.}
...
Writing the Fox response through a path-length power law assigns every system $L=600$ kpc. Consequently that sample identifies one effective cluster amplitude, not the exponent of a general path-length functional. I could not supply a unique operational definition of $L$ for an arbitrary three-dimensional baryonic distribution. I therefore removed $A(L)$ from the predictive response and removed conclusions that used it to connect disks and clusters, rather than replacing it with a more flexible fitted law; path length is retained only as a hypothesis.
...
The no-refit radial check uses the Tian et al. CLASH radial-acceleration catalog \cite{ref9}. Cluster names are lowercased, stripped of non-alphanumeric characters, normalized for Abell and MACS aliases, and excluded only on exact equality with a Fox calibration name. This rule excludes MACS0416, MACS0717, and MACS1149, leaving 73 measurements in 17 clusters. The submitted Fox-calibrated $B_{\rm Fox}=8.446$, $h(g_N)$, and $g^\dagger$ are frozen before comparison. Repeated radii are retained within cluster and complete clusters are resampled for uncertainty; a weighted trend of log residual against $\log_{10}(r/200\ {\rm kpc})$ uses 5,000 cluster-bootstrap draws.
