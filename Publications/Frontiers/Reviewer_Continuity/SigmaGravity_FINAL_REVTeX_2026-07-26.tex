...
The no-refit radial check uses the Tian et al. CLASH radial-acceleration catalog \cite{ref9}. Cluster names are lowercased, stripped of non-alphanumeric characters, normalized for Abell and MACS aliases, and excluded only on exact equality with a Fox calibration name. This rule excludes MACS0416, MACS0717, and MACS1149, leaving 73 measurements in 17 clusters. The Fox-calibrated $B_{\rm Fox}=8.446$, $h(g_N)$, and $g^\dagger$ are fixed before comparison. Repeated radii are retained within cluster and complete clusters are resampled for uncertainty; a weighted trend of log residual against $\log_{10}(r/200\ {\rm kpc})$ uses 5,000 cluster-bootstrap draws.
