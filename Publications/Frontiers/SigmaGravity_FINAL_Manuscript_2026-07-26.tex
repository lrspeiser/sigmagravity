...
\begin{center}
\small Manuscript ID 1866133 \quad Word count: 4302 \quad Figures: 4 \quad Tables: 2
\end{center}
...
The conclusion is sensitive to common nuisance choices. The central configuration remains the primary result and was not selected from the 81-case grid. Across those frozen combinations, including common changes in \(A_0\), the mean paired contrast ranges from \(-3.286\) to \(+2.779\ {\rm km\,s^{-1}}\) and retains the central sign in 64.2\% of cases (Figure~\ref{fig:1}). Changing \(\sigma\) from the central 20 km s\(^{-1}\) to 10, 30, and 50 km s\(^{-1}\) gives mean \(\Sigma\)-Gravity RMS values of 16.695, 16.347, and 17.600 km s\(^{-1}\), respectively, compared with 16.366 km s\(^{-1}\) at 20 km s\(^{-1}\). The 30 km s\(^{-1}\) value is only 0.019 km s\(^{-1}\) lower than the central choice, and no alternative is selected. All four \(\sigma\) choices retain a positive mean \(\Sigma\)-minus-MOND contrast. These sensitivities show that the fixed choices are not uniquely identified and are a reason not to overinterpret the small difference between the prescriptions.
...
Equation (16) is used to calibrate the common cluster response on the 42 Fox systems, giving a median predicted-to-observed mass ratio of 0.987 and a scatter of 0.132 dex (Figure~\ref{fig:2}, left). Because the amplitude and simplified baryon proxy are inseparable in this calculation, the exercise is retained only as an illustrative in-sample calibration. Repeated Fox holdouts measure stability under resampling of the same catalog; they do not constitute independent validation.
