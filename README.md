...
The repository now includes the reproducible [Gravity Theory Compiler](research/sigma-theory-compiler/README.md): a bounded covariant-action generator with Cadabra tensor controls, ADM/Dirac analysis, principal-symbol and Hamiltonian gates, GPU-oriented enumeration, observation seals, and hash-bound evidence artifacts. Its current production ledger passes 100/100 formal controls. The first 12 fixed-coefficient linear-`X` quartic Horndeski candidates have exact all-direction local strong-hyperbolicity certificates, forward-invariant homogeneous ADM/Dirac/quadratic-Hamiltonian branches, finite-horizon all-wavenumber linearized physical-energy estimates, bounded linear lapse/shift reconstruction through one time derivative, uniform fourth-order derivative envelopes for the exact quasilinear companion coefficients, and an exact 55-variable physical-space principal first-order reduction; they are not observationally promoted, because nonlinear state-to-jet/source and gauge reconstruction, full nonlinear inhomogeneous PDE trapping, and nonlinear global energy remain open.
