...
This is a LEGACY regression test script kept for historical reference only.

USE INSTEAD: scripts/run_regression.py
    - This is the official regression test referenced in SUPPLEMENTARY_INFORMATION.md
    - Uses canonical first-principles parameters (A₀=e^(1/2π), ξ=R_d/(2π))
    - Produces results documented in SI §4

This old script has multiple configurations including legacy parameters (√3, ξ=2/3)
that are no longer the canonical model. Do not use for validation.
...
Usage:
    python derivations/old_do_not_use_full_regression_test.py [--verbose] [--quick]
