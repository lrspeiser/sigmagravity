#!/usr/bin/env python3
"""
Evaluate V2.1 Enhanced B/T Laws on SPARC Sample
===============================================

Tests two critical fixes:
1. Lambda depends on BOTH B/T and shear S (prevents too-long coherence)
2. Ring term has radial Gaussian concentration (prevents broad overshoot)

These target the systematic "red overshoot" failures in intermediate spirals.

Usage:
    python many_path_model/bt_law/evaluate_bt_laws_v2p1.py
"""
import argparse
import json
import sys
...
from bt_laws_v2p1 import load_theta, eval_all_laws_v2p1, ring_radial_envelope


def compute_galaxy_ape_v2p1(galaxy: SPARCGalaxy, params: dict) -> float:
    """
    Compute APE using V2.1 many-path model with:
    - Kappa coherence factor
    - Radial ring concentration envelope
    
    Args:
        galaxy: SPARC galaxy data
        params: Parameter dict with eta, ring_amp, M_max, lambda_ring, kappa, R_ring, etc.
    ...
    # Many-path multiplier M(r) with V2.1 enhancements
    eta = params['eta']
    ...
    # NEW: Radial concentration envelope
    # Convert to numpy for envelope computation, then back to cupy
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate V2.1 Enhanced B/T Laws on SPARC')
    parser.add_argument('--bt_params', type=Path,
                       default=Path('many_path_model/bt_law/bt_law_params_v2p1.json'),
                       help='Path to v2.1 B/T law parameters')
    ...
    parser.add_argument('--output_dir', type=Path,
                       default=Path('results/bt_law_evaluation_v2p1'),
                       help='Output directory for results')
    parser.add_argument('--use_v2_params', action='store_true',
                       help='Use v2 fitted params as starting point instead of defaults')
    
    args = parser.parse_args()
    
    # Load or create parameters
    print("="*80)
    print("V2.1 ENHANCED B/T LAW EVALUATION ON FULL SPARC SAMPLE")
    print("="*80)
    
    if args.use_v2_params and Path('many_path_model/bt_law/bt_law_params_v2.json').exists():
        print(f"\nLoading v2 fitted parameters and upgrading to v2.1...")
        from bt_laws_v2p1 import default_theta_v2p1
        theta = load_theta('many_path_model/bt_law/bt_law_params_v2.json')
        # Add v2.1 specific params
        defaults_v2p1 = default_theta_v2p1()
        theta['lambda_ring'] = defaults_v2p1['lambda_ring']
        theta['S0_lambda'] = defaults_v2p1['S0_lambda']
        theta['b0_ring'] = defaults_v2p1['b0_ring']
        theta['b1_ring'] = defaults_v2p1['b1_ring']
        theta['sigma_ring'] = defaults_v2p1['sigma_ring']
    elif args.bt_params.exists():
        print(f"\nLoading v2.1 B/T law parameters from: {args.bt_params}")
        theta = load_theta(args.bt_params)
    else:
        print(f"\nUsing default v2.1 parameters")
        from bt_laws_v2p1 import default_theta_v2p1
        theta = default_theta_v2p1()
    ...
    print("\n" + "="*80)
    print("EVALUATING GALAXIES WITH V2.1 LAWS")
    print("="*80)
    ...
    for i, rotmod_file in enumerate(rotmod_files, 1):
        ...
        try:
            ...
            # Predict parameters using v2.1 laws
            v2p1_params = eval_all_laws_v2p1(B_T, theta,
                                             Sigma0=Sigma0,
                                             R_d=R_d,
                                             shear=shear)
            
            # Compute APE with v2.1 parameters
            v2p1_ape = compute_galaxy_ape_v2p1(galaxy, v2p1_params)
            ...
            result = {
                ...
                'delta_ape': v2p1_ape - per_gal_ape if per_gal_ape else None,
                'v2p1_params': v2p1_params,
            ...
            status += f" B/T={B_T:.2f}"
            if Sigma0:
                status += f" Σ={Sigma0:.0f}"
            if shear:
                status += f" S={shear:.2f}"
            status += f" λ={v2p1_params['lambda_ring']:.1f}"
            status += f" | V2.1: {v2p1_ape:6.2f}%"
            if per_gal_ape:
                status += f" | PerGal: {per_gal_ape:6.2f}% | Δ: {v2p1_ape - per_gal_ape:+6.2f}%"
            print(status)
    ...
    # V2.1 Law statistics
    v2p1_apes = [r['v2p1_ape'] for r in results]
    print("\n" + "-"*80)
    print("V2.1 ENHANCED B/T LAW PERFORMANCE")
    print("  (Two-Predictor Lambda + Ring Concentration)")
    print("-"*80)
    print(f"  Mean APE:       {np.mean(v2p1_apes):.2f}%")
    print(f"  Median APE:     {np.median(v2p1_apes):.2f}%")
    print(f"  Std Dev:        {np.std(v2p1_apes):.2f}%")
    print(f"  Min APE:        {np.min(v2p1_apes):.2f}%")
    print(f"  Max APE:        {np.max(v2p1_apes):.2f}%")
    
    # Quality distribution
    excellent = sum(1 for ape in v2p1_apes if ape < 10)
    good = sum(1 for ape in v2p1_apes if 10 <= ape < 20)
    fair = sum(1 for ape in v2p1_apes if 20 <= ape < 30)
    poor = sum(1 for ape in v2p1_apes if ape >= 30)
    
    print(f"\nQuality Distribution:")
    print(f"  Excellent (< 10%):   {excellent:3d} ({100*excellent/len(v2p1_apes):.1f}%)")
    print(f"  Good (10-20%):       {good:3d} ({100*good/len(v2p1_apes):.1f}%)")
    print(f"  Fair (20-30%):       {fair:3d} ({100*fair/len(v2p1_apes):.1f}%)")
    print(f"  Poor (≥ 30%):        {poor:3d} ({100*poor/len(v2p1_apes):.1f}%)")
    ...
    if valid_comparisons:
        ...
        print(f"\n  Mean Δ (V2.1 - PerGal):  {np.mean(deltas):+.2f}%")
    ...
    # Full results JSON
    output_file = args.output_dir / 'v2p1_evaluation_results.json'
    output_data = {
        'bt_law_version': 'v2.1_enhanced',
        'bt_law_params': theta,
        'evaluation_time': elapsed,
        'total_galaxies': len(rotmod_files),
        'successful': len(results),
        'v2p1_statistics': {
            'mean_ape': float(np.mean(v2p1_apes)),
            'median_ape': float(np.median(v2p1_apes)),
            'std_ape': float(np.std(v2p1_apes)),
            'min_ape': float(np.min(v2p1_apes)),
            'max_ape': float(np.max(v2p1_apes))
        },
        'results': results
    }
    ...
    # Summary CSV
    csv_file = args.output_dir / 'v2p1_evaluation_summary.csv'
