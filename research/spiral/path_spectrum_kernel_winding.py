...
class PathSpectrumHyperparams:
    ...
    # NEW: Spiral winding parameters (tuned for RAR)
    use_winding: bool = True  # Enable spiral winding gate
    N_crit: float = 100.0     # Critical winding number (tuned: 100-150 optimal for RAR)
    t_age: float = 10.0       # System age [Gyr]
    wind_power: float = 1.0   # Winding suppression steepness (linear is gentler)
...
class PathSpectrumKernel:
    ...
    def winding_suppression(self, r: Union[float, np.ndarray],
                           v_circ: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        """Spiral winding suppression gate
        
        After ~N_crit orbits, coherent paths wind so tightly that adjacent
        paths interfere destructively. This naturally suppresses fast rotators
        (massive spirals) more than slow rotators (dwarfs).
        
        G_wind = 1 / (1 + (N_orbits/N_crit)^wind_power)
        
        where N_orbits = t_age × v_c / (2πR × 0.978)
        ...
        Returns:
        --------
        G_wind : float or array
            Winding gate [0, 1], where 1 = no suppression
        
        Physical derivation of N_crit:
        ...
        - Therefore: N_crit ~ v_c/σ_v ~ 200/20 = 10
        """
        if not self.hp.use_winding:
            r_arr = self.xp.asarray(r)
            return self.xp.ones_like(r_arr)
        
        r_arr = self.xp.asarray(r)
        v_arr = self.xp.asarray(v_circ)
        
        # Protect against edge cases
        r_safe = self.xp.maximum(r_arr, 0.1)
        v_safe = self.xp.maximum(v_arr, 1.0)
        
        # Number of orbits: N = t_age × v_c / (2πR × 0.978)
        # 0.978 Gyr converts kpc·km/s to Gyr
        N_orbits = self.hp.t_age * v_safe / (2.0 * self.xp.pi * r_safe * 0.978)
        
        # Winding suppression
        G_wind = 1.0 / (1.0 + self.xp.power(N_orbits / self.hp.N_crit, self.hp.wind_power))
    ...
    def many_path_boost_factor(self, r: Union[float, np.ndarray],
        ...
        # NEW: Spiral winding suppression (morphology-dependent)
        G_wind = self.winding_suppression(r, v_circ)
        
        # Combined kernel:
        # K = A_0 * RAR_shape * coherence * small_r_gate * winding
        K_total = self.hp.A_0 * K_rar * K_coherence * S_sm * G_wind
