...
SCHEMA_VERSION = "sigma-quartic-tc2-second-atom-chunk128-campaign-1.0"
CHUNK_OFFSET = 128


class QuarticTC2SecondAtomChunk128Error(ValueError):
    """Raised when the offset-128 continuation is not exactly reproducible."""


def generic_second_atom_chunk128_boundary_control() -> tuple[bool, dict[str, Any]]:
    prior_offset = 64
    prior_size = DEFAULT_CHUNK_SIZE
    expected_offset = prior_offset + prior_size
    corrupted_offset = expected_offset + 1
    passed = bool(expected_offset == CHUNK_OFFSET and corrupted_offset != CHUNK_OFFSET)
    return passed, {
        "control": "contiguous selector-window continuation from offset 64 to 128",
        "prior_offset": prior_offset,
        "prior_size": prior_size,
        "expected_next_offset": expected_offset,
        "negative_controls": {
            "skip_one_selector_pair": {
                "corrupted_offset": corrupted_offset,
                "required_offset": CHUNK_OFFSET,
                "rejected": corrupted_offset != CHUNK_OFFSET,
            },
            "claim_192_pairs_are_full_coverage": {
                "cumulative": 3 * DEFAULT_CHUNK_SIZE,
...
def run_quartic_tc2_second_atom_chunk128_campaign(
    ...
    try:
        if config.get("schema_version") != SCHEMA_VERSION:
            raise QuarticTC2SecondAtomChunk128Error(
        ...
        if prior_chunk_campaign.get("status") != (
            ...
            raise QuarticTC2SecondAtomChunk128Error("prior chunk status mismatch")
        if variable_campaign.get("status") != (
            "pass_all_12_first_order_variable_deltaK_extensions_"
            "higher_orders_global_H7_fail_closed"
        ):
            raise QuarticTC2SecondAtomChunk128Error("variable campaign status mismatch")
        if not _content_hash_matches(prior_chunk_campaign) or not _content_hash_matches(
            variable_campaign
        ):
            raise QuarticTC2SecondAtomChunk128Error("campaign content hash mismatch")
        ...
        if (
            ...
            or prior_contract["chunk_offset"] != 64
        ...
        if not prior_chain_passed:
            raise QuarticTC2SecondAtomChunk128Error("prior record chain mismatch")
        ...
        if not (generic_passed and resume_passed and boundary_passed):
            raise QuarticTC2SecondAtomChunk128Error(
        ...
        if len(coefficients_by_candidate) != 12:
            raise QuarticTC2SecondAtomChunk128Error("candidate count mismatch")
        seed = _content_hash(
            {
        ...
        body = {
            ...
            "status": (
                ...
                else "exact_second_atom_Sylvester_obstruction_found_in_chunk128_global_H7_fail_closed"
            ),
            "errors": [],
            "upstream_sha256": {
            ...
            "claim": (
                "The offset-64 chain was revalidated and selector indices 128 onward were "
                "evaluated until completion or first obstruction. Unevaluated pairs are not inferred."
            ),
            "scope": (
                "Cumulative second-order Sylvester algebra through selector index 191 only. "
                "TC2, B7, global H7, dyadic summation, and lifespan remain fail-closed."
            ),
        }
    except (KeyError, TypeError, ValueError, QuarticTC2SecondAtomChunk128Error) as error:
...
def write_quartic_tc2_second_atom_chunk128_campaign(
