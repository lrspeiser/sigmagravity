...
def build_output_row_arithmetic_packet(
    output_row: int, component_provenance_hash: str
) -> dict[str, Any]:
    if not 0 <= output_row < DIMENSION:
        raise ValueError("output row must be in 0..10")
    dag = ArithmeticDag()
    matrix = [
        [
