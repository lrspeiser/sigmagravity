    ...
    for candidate_id in sorted(action_map):
        ...
        if preflight_record["decision"] == "reject":
            ...
            formal_scope = (
                ...
                "source-energy margins. An exact weighted-IFT contract gate now records the eleven "
                "candidate-bound norm, gauge, map, inverse, remainder, and completed-boundary fields "
                "that are still absent for each of those three candidates; no constants are assumed. "
                "They therefore remain blocked before a full weighted elliptic inverse, nonlinear "
                "Frechet remainder, or completed-boundary sign theorem. The other 11 retain the "
            ...
            formal_scope = (
                ...
                "pieces. Exact Hamiltonian reduction and a nonradial Green comparison now extend "
                "the rejected class beyond |K| <= (6/5)v to candidate-bound grid caps 1.211, 1.211, "
                "and 1.210. The immediately following 0.001 grid point is explicitly inconclusive "
                "for every candidate. Non-conformally-flat geometry, mean curvature outside each "
                "registered cap, or different "
