    ...
    for candidate_id in sorted(action_map):
        ...
        if preflight_record["decision"] == "reject":
            ...
            formal_scope = (
                ...
                "source-energy margins. On concrete H2_-1/2 to L2_-5/2 weighted spaces, a fixed "
                "conformal/York gauge gives an exact metric reference principal spectrum "
                "(2, 2, 8/3, 4), ellipticity margin 2, and trivial decaying reference kernel; the "
                "registered compact-source right inverses also carry through. The candidate-specific "
                "Aether constraint-variable block and its metric-Aether off-diagonal principal symbol "
                "remain absent, so no full Fredholm/isomorphism, inverse-norm, nonlinear-remainder, "
                "or completed-boundary sign theorem is inferred. The other 11 retain the "
            ...
            formal_scope = (
                ...
                "pieces. Exact Hamiltonian reduction and a nonradial Green comparison now yield each "
                "candidate's unique positive algebraic threshold kappa_star, the root of the exact "
                "quadratic Green bound. The closed class |K| <= kappa_star*v is excluded, including "
                "the endpoint by strict interior-source and Newton-kernel inequalities; exact rational "
                "controls above each threshold remain explicitly inconclusive. Non-conformally-flat "
                "geometry, mean curvature above the analytic threshold, or different "
