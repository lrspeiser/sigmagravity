...
SCHEMA_VERSION = "sigma-quartic-tc2-second-atom-chunk64-campaign-1.0"
CHUNK_OFFSET = 64


class QuarticTC2SecondAtomChunk64Error(ValueError):
    """Raised when the offset-64 second-atom continuation is not reproducible."""
...
def _candidate_coefficients(
    ...
    return {
        str(certificate["candidate_id"]): certificate["coefficients"]
        for certificate in variable_campaign.get("certificates", [])
    }


def _validate_prior_chain(prior: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    contract = prior["chunk_contract"]
    previous = contract["chunk_seed_sha256"]
    records = prior["pair_manifest"]
    failures: list[int] = []
    ...
    passed = bool(
        not failures
        and len(records) == DEFAULT_CHUNK_SIZE
        and previous == contract["resume_after_record_sha256"]
    )
    return passed, {
        "records_checked": len(records),
        "failure_indices": failures,
        "computed_resume_sha256": previous,
        "declared_resume_sha256": contract["resume_after_record_sha256"],
        "passed": passed,
    }


def generic_second_atom_chunk64_resume_control() -> tuple[bool, dict[str, Any]]:
    seed = "seed"
    first_body = {"index": 0, "previous_record_sha256": seed, "value": "A"}
    first_hash = _content_hash(first_body)
    second_body = {
        "index": 1,
        "previous_record_sha256": first_hash,
        "value": "B",
    }
    second_hash = _content_hash(second_body)
    corrupted = dict(second_body)
    corrupted["previous_record_sha256"] = seed
    passed = bool(
        second_body["previous_record_sha256"] == first_hash
        and _content_hash(second_body) == second_hash
        and _content_hash(corrupted) != second_hash
    )
    return passed, {
        "control": "offset-64 hash-chain continuation boundary",
        "first_record_sha256": first_hash,
        "second_record_sha256": second_hash,
        "negative_controls": {
            "restart_from_seed_instead_of_prior_tip": {
                "corrupted_sha256": _content_hash(corrupted),
                "required_sha256": second_hash,
                "rejected": _content_hash(corrupted) != second_hash,
            },
            "claim_continuation_is_full_coverage": {
                "cumulative_after_chunk": 2 * DEFAULT_CHUNK_SIZE,
                "total": ATOM_DIMENSION * (ATOM_DIMENSION + 1) // 2,
                "rejected": True,
            },
            "promote_chunk_to_global_H7": {
                "missing": "remaining second-atom pairs, TC2 energy ledger, CK1/CK3",
                "rejected": True,
            },
        },
        "passed": passed,
...
def _substitute_entries(
    entries: list[dict[str, Any]], substitutions: dict[sp.Symbol, sp.Expr]
) -> list[dict[str, Any]]:
    result: list[dict[str, Any]] = []
    for entry in entries:
        value = sp.factor(sp.sympify(entry["value"]).subs(substitutions))
        if value != 0:
            result.append({**entry, "value": str(value)})
    return result


def run_quartic_tc2_second_atom_chunk64_campaign(
    ...
    try:
        if config.get("schema_version") != SCHEMA_VERSION:
            raise QuarticTC2SecondAtomChunk64Error(
        ...
        if prior_chunk_campaign.get("status") != (
            ...
            raise QuarticTC2SecondAtomChunk64Error("prior chunk status mismatch")
        if variable_campaign.get("status") != (
            "pass_all_12_first_order_variable_deltaK_extensions_"
            "higher_orders_global_H7_fail_closed"
        ):
            raise QuarticTC2SecondAtomChunk64Error("variable campaign status mismatch")
        if not _content_hash_matches(prior_chunk_campaign) or not _content_hash_matches(
            variable_campaign
        ):
            raise QuarticTC2SecondAtomChunk64Error("campaign content hash mismatch")
        ...
        if (
            ...
            or prior_contract["chunk_offset"] != 0
            or prior_contract["chunk_size"] != DEFAULT_CHUNK_SIZE
        ...
        if not prior_chain_passed:
            raise QuarticTC2SecondAtomChunk64Error("prior record chain mismatch")
        ...
        if not generic_passed or not resume_passed:
            raise QuarticTC2SecondAtomChunk64Error("generic continuation control failed")
        ...
        if len(coefficients_by_candidate) != 12:
            raise QuarticTC2SecondAtomChunk64Error("candidate count mismatch")
        seed = _content_hash(
            {
        ...
        evaluated_cumulative = int(
            prior_chunk_campaign["counts"]["evaluated_coordinate_atom_pairs"]
        ) + evaluated_current
        ...
        body = {
            ...
            "status": (
                ...
                else "exact_second_atom_Sylvester_obstruction_found_in_chunk64_global_H7_fail_closed"
            ),
            "errors": [],
            "upstream_sha256": {
            ...
            "exact_tensor_summary_current_chunk": tensor_summary,
            "first_exact_obstruction": first_obstruction,
            "counts": {
                "total_unordered_coordinate_atom_pairs": total_pairs,
                "prior_evaluated_coordinate_atom_pairs": int(
                    prior_chunk_campaign["counts"]["evaluated_coordinate_atom_pairs"]
                ),
                ...
                "cumulative_deltaK_AB_constructions": int(
                    prior_chunk_campaign["counts"]["deltaK_AB_constructions"]
                )
                + solvable_current,
                "TC2_closures": 0,
            ...
            "claim": (
                "The prior 64-record chain was revalidated and the next declared selector "
                "chunk was evaluated. No result is inferred for unevaluated pairs."
            ),
            "scope": (
                "Cumulative second-order Sylvester algebra through selector offset 127 only. "
                "TC2, B7, global H7, dyadic summation, and lifespan remain fail-closed."
            ),
        }
    except (KeyError, TypeError, ValueError, QuarticTC2SecondAtomChunk64Error) as error:
        ...
        body = {
            ...
            "counts": {
                "total_unordered_coordinate_atom_pairs": total_pairs,
                "prior_evaluated_coordinate_atom_pairs": 0,
...
def write_quartic_tc2_second_atom_chunk64_campaign(
