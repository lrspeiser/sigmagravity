...
SCHEMA_VERSION = "sigma-quartic-tc2-second-atom-chunk192-campaign-1.0"
CHUNK_OFFSET = 192


class QuarticTC2SecondAtomChunk192Error(ValueError):
    """Raised when the offset-192 continuation is not exactly reproducible."""


def generic_second_atom_chunk192_boundary_control() -> tuple[bool, dict[str, Any]]:
    prior_offset = 128
    expected = prior_offset + DEFAULT_CHUNK_SIZE
    skipped = expected + 1
    passed = bool(expected == CHUNK_OFFSET and skipped != CHUNK_OFFSET)
    return passed, {
        "control": "contiguous selector continuation from offset 128 to 192",
        "prior_offset": prior_offset,
        "prior_size": DEFAULT_CHUNK_SIZE,
        "expected_next_offset": expected,
        "negative_controls": {
            "skip_one_selector_pair": {
                "corrupted_offset": skipped,
                "required_offset": CHUNK_OFFSET,
                "rejected": skipped != CHUNK_OFFSET,
            },
            "claim_256_pairs_are_full_coverage": {
                "cumulative": 4 * DEFAULT_CHUNK_SIZE,
...
def _tensor_summary(packets: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "unique_direction_pair_packets": len(packets),
        "nonzero_D2P55_packets": sum(
            packet["D2P55_nonzero_entries"] > 0 for packet in packets
        ),
        "nonzero_D2K55_packets": sum(
            packet["D2K55_nonzero_entries"] > 0 for packet in packets
        ),
        "nonzero_D2TC2_packets": sum(
            packet["D2TC2_nonzero_entries"] > 0 for packet in packets
        ),
        "nonzero_deltaK_AB_packets": sum(
            packet["deltaK_AB_nonzero_entries"] > 0 for packet in packets
        ),
        "maximum_deltaK_AB_rank": max(
            packet["deltaK_AB_rank"] for packet in packets
        ),
        "all_deltaK_AB_Hermitian": all(
            packet["deltaK_AB_Hermitian"] for packet in packets
        ),
        "all_second_Sylvester_residuals_zero": all(
            packet["second_Sylvester_residual_zero"] for packet in packets
        ),
    }


def run_quartic_tc2_second_atom_chunk192_campaign(
    ...
    try:
        if config.get("schema_version") != SCHEMA_VERSION:
            raise QuarticTC2SecondAtomChunk192Error(
        ...
        if prior_chunk_campaign.get("status") != (
            ...
            raise QuarticTC2SecondAtomChunk192Error("prior chunk status mismatch")
        if variable_campaign.get("status") != (
            "pass_all_12_first_order_variable_deltaK_extensions_"
            "higher_orders_global_H7_fail_closed"
        ):
            raise QuarticTC2SecondAtomChunk192Error("variable campaign status mismatch")
        if not _content_hash_matches(prior_chunk_campaign) or not _content_hash_matches(
            variable_campaign
        ):
            raise QuarticTC2SecondAtomChunk192Error("campaign content hash mismatch")
        ...
        if (
            ...
            or prior_contract["chunk_offset"] != 128
        ...
        if not prior_chain_passed:
            raise QuarticTC2SecondAtomChunk192Error("prior record chain mismatch")
        generic_passed, generic = generic_second_atom_sylvester_control()
        boundary_passed, boundary = generic_second_atom_chunk192_boundary_control()
        ...
        if len(coefficients_by_candidate) != 12:
            raise QuarticTC2SecondAtomChunk192Error("candidate count mismatch")
        seed = _content_hash(
            {
        ...
        body = {
            ...
            "status": (
                ...
                else "exact_second_atom_Sylvester_obstruction_found_in_chunk192_global_H7_fail_closed"
            ),
            "errors": [],
            "upstream_sha256": {
            ...
            "claim": (
                "The offset-128 chain was revalidated and selector indices 192 onward were "
                "evaluated until completion or first obstruction. Unevaluated pairs are not inferred."
            ),
            "scope": (
                "Cumulative second-order Sylvester algebra through selector index 255 only. "
                "TC2, B7, global H7, dyadic summation, and lifespan remain fail-closed."
            ),
        }
    except (KeyError, TypeError, ValueError, QuarticTC2SecondAtomChunk192Error) as error:
...
def write_quartic_tc2_second_atom_chunk192_campaign(
