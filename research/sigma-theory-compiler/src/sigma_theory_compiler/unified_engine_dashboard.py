...
<section><h2>Kastner–Schlatter transactional-gravity proposal</h2><div class="metrics">{_metric("Decision", transactional["decision"])}{_metric("Equation checks", transactional["equation_preflight_counts"]["pass"])}{_metric("Equation blocks", transactional["equation_preflight_counts"]["block"])}{_metric("Graph nodes", transactional["equation_graph"]["counts"]["nodes"])}{_metric("Graph edges", transactional["equation_graph"]["counts"]["edges"])}{_metric("Poisson samples", transactional["cuda_consequence_campaign"]["counts"]["poisson_samples"])}{_metric("SdS cases", transactional["cuda_consequence_campaign"]["counts"]["sds_cases"])}{_metric("MOND cases", transactional["cuda_consequence_campaign"]["counts"]["mond_cases"])}{_metric("CUDA evaluations", transactional["cuda_consequence_campaign"]["counts"]["gpu_measured_consequence_evaluations"])}{_metric("CUDA throughput", f'{transactional["cuda_consequence_campaign"]["runtime_measurement"]["gpu_consequence_evaluations_per_second"] / 1e9:.2f}B/s')}</div><p>The source-bound equation intake and typed graph represent the proposal as 25 formula nodes with explicit assumptions, domains, dependencies, and absent-action edges. The RTX campaign reproduces bounded Poisson four-volume statistics, Schwarzschild–de Sitter root/domain consequences, and the conditional MOND/BTFR asymptote with exact CPU controls. Equation 35 remains blocked because substituting <code>h=2*pi*hbar</code> into its middle expression gives <code>8*pi^2 l_P^2 q</code>, while the printed final coefficient is <code>4*pi^2</code>; no Lambda value or cosmology-derived <code>a0</code> was emitted. First broader blocker: {_escape(transactional["first_blocker"])}. Synthetic agreement does not establish the transactional ontology, a fundamental action, GR equivalence, dark-sector elimination, or observational validity.</p></section>
...
<section><h2>Current exact Aether and G3 boundary</h2><div class="metrics">{_metric("Aether C3 jet envelopes", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["compact_profile_C3_weighted_jet_bound_pass_count"])}{_metric("Aether coefficient contracts", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["lower_order_coefficient_contract_declared_count"])}{_metric("Aether canonical backgrounds", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["full_canonical_background_point_registered_count"])}{_metric("Aether D residual DAGs", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["candidate_bound_flat_chart_D_residual_DAG_registered_count"])}{_metric("Aether H-core registries", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["spatially_distributed_canonical_H_core_registered_count"])}{_metric("Aether covariant H/D DAGs", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["metric_covariantized_H_D_Frechet_DAG_registered_count"])}{_metric("G3 radial momentum audits", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["radial_momentum_leading_order_pass_count"])}{_metric("G3 Hamiltonian audits", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["flat_Hamiltonian_leading_order_pass_count"])}{_metric("Real joint coefficients", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["joint_real_asymptotic_coefficient_solution_count"])}{_metric("Flat radial classes rejected", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["flat_radial_matched_constraint_class_reject_count"])}</div><p>The sole uniformly elliptic Aether candidate now has an exact compact C3 profile jet envelope, a typed lower-order coefficient interface, and a candidate-bound flat-chart canonical seed containing <code>q</code>, <code>A</code>, <code>pi^ij</code>, and <code>p_A^i</code>. Its distributed cotangent-lift momentum residual DAG is exact through the registered profile derivatives. The spatially distributed canonical <code>H_core</code> and off-flat metric-covariantized Hamiltonian/momentum Frechet DAG remain absent, so the complete B/C registry, weighted Fredholm inverse, nonlinear remainder, completed-boundary sign, and candidate rejection remain blocked. For the three future G3 actions, real flat spherical AF York data with standard r^-2 falloff obey <code>alpha+2k=0</code> and <code>1-(2/3)k^2+(2/3)alpha^2=0</code>, hence <code>1+2k^2=0</code>. This excludes that matched data class, not the actions or theories. Nonflat, nonradial, different-falloff, and logarithmic or polyhomogeneous data remain open.</p></section>
  {future_dossier_html}
...
<section><h2>Fourth-order range closure</h2><div class="metrics">{_metric("Exact selector", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["selector_obligations"])}{_metric("Records evaluated", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["obligations_evaluated"])}{_metric("Obligations closed", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["obligations_closed"])}{_metric("Unevaluated after stop", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["obligations_remaining"])}{_metric("Candidate checks solved", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["candidate_solvable"])}{_metric("Candidate obstructions", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["candidate_obstructed"])}{_metric("Canonical candidates obstructed", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["canonical_obstruction_certificate"]["candidate_obstructions_certified"])}{_metric("Cokernel rank", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["canonical_obstruction_certificate"]["zero_eigenspace_compression_rank"])}{_metric("Alternative lower jets ruled out", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["canonical_obstruction_certificate"]["alternative_lower_jet_homogeneous_completion_ruled_out"])}{_metric("Permanent stop", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["permanently_stopped"])}{_metric("Parallel workers", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["parallel_worker_count"])}</div><p>The selector is the exact minimal fourth-order range basis induced by the completed rank-15 cubic sector. The service closed 244 obligations, then the record at selector offset 244 and active indices [0,2,3,9] failed canonical fourth-order equal-eigenspace Sylvester compatibility for all 12 quartic candidates. Its only nonzero reference compression factors exactly as (34816/15)*alpha^5*W, with rank-two W, so the canonical recurrence is compatible iff alpha=0 and cannot be repaired by c20; every registered candidate has nonzero alpha. This does not yet rule out a correction from unparameterized order-one-through-three equal-eigenspace homogeneous Sylvester freedom. The service committed no later records and stopped permanently; the remaining 2,816 obligations are unevaluated rather than inferred. Full fourth-order range closure, CK1, CK3, TC2, B7, global H7, and lifespan therefore remain false.</p></section>
