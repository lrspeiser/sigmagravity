...
<section><h2>Live dashboard refresh service</h2><div class="metrics">{_metric("Readiness", continuous_dashboard["decision"])}{_metric("Availability", live_dashboard.get("availability"))}{_metric("State", live_dashboard.get("state"))}{_metric("Alive", live_dashboard.get("alive"))}{_metric("Refreshes", live_dashboard.get("refresh_count", 0))}{_metric("Refresh interval", str(continuous_dashboard["refresh_interval_seconds"]) + "s")}{_metric("Maximum refreshes", continuous_dashboard["maximum_refreshes"])}{_metric("Consecutive failures", live_dashboard.get("consecutive_failures", 0))}{_metric("Safety implementation", continuous_dashboard["safety_hardening"]["decision"])}{_metric("Safety service started", continuous_dashboard["safety_hardening"]["service_started"])}{_metric("Control poll", str(continuous_dashboard["safety_hardening"]["safety_contract"]["control_poll_interval_seconds"]) + "s")}{_metric("Refresh guards", continuous_dashboard["safety_hardening"]["safety_contract"]["refresh_phase_control_guards"])}</div><p>This volatile panel is excluded from the deterministic core. The active bounded service opens the campaign database with the read-only/query-only contract, publishes atomically into an ignored runtime directory, and never overwrites the immutable checked snapshot. A separate hardened replacement now binds Windows-safe argv/PID identity, counter-preserving reload epochs, interruptible polling, unpredictable exclusive temporary files, symlink rejection, and pre/post dependency manifests. It is ready but deliberately not substituted for the currently running service; individual snapshot and dashboard replacements are atomic, but not one cross-file transaction.</p></section>
