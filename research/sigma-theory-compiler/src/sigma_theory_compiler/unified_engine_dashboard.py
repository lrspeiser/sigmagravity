...
<section><h2>Current exact Aether and G3 boundary</h2><div class="metrics">{_metric("Aether finite-tilt symbols", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["finite_tilt_metric_York_symbol_derived_count"])}{_metric("Aether uniform ellipticity", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["uniform_fixed_free_data_principal_ellipticity_pass_count"])}{_metric("Aether York shells", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["exact_nonelliptic_York_shell_count"])}{_metric("G3 general-geometry theorems", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["general_geometry_pointwise_theorem_pass_count"])}{_metric("G3 curvature-shortfall classes rejected", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["curvature_shortfall_constraint_class_reject_count"])}{_metric("Constructed nonconformal metrics", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["nonconformally_flat_metric_construction_pass_count"])}</div><p>The candidate-bound Aether finite-tilt metric-to-York symbol is now exact for all three regular seeds: one is uniformly principal elliptic on its registered amplitude range, while two cross exact perpendicular-covector shells. Those two results reject only the global York completion variable, not the theories. The G3 pointwise theorem now permits arbitrary smooth complete AF three-geometry and unrestricted Cotton tensor, but excludes only compensated data with a finite scalar-curvature shortfall below the exact threshold. Endpoint and above-threshold geometries remain inconclusive, and no nonconformally-flat solution is inferred.</p></section>
  {future_dossier_html}
...
<section><h2>Fourth-order range closure</h2><div class="metrics">{_metric("Exact selector", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["selector_obligations"])}{_metric("Obligations evaluated", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["obligations_evaluated"])}{_metric("Obligations remaining", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["obligations_remaining"])}{_metric("Candidate checks solved", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["candidate_solvable"])}{_metric("Candidate obstructions", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["candidate_obstructed"])}{_metric("Parallel workers", core["quartic_nonlinear_closure"]["fourth_jet_range_obligations"]["parallel_worker_count"])}</div><p>The selector is the exact minimal fourth-order range basis induced by the completed rank-15 cubic sector. Each displayed count is backed by a hash-chained symbolic record. Unevaluated obligations are never inferred to pass, and fourth-order completion does not by itself promote CK1, CK3, TC2, B7, global H7, or lifespan.</p></section>
