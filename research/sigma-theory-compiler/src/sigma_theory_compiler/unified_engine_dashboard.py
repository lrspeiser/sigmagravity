...
<section><h2>Current exact Aether and G3 boundary</h2><div class="metrics">{_metric("Aether finite-tilt symbols", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["finite_tilt_metric_York_symbol_derived_count"])}{_metric("Aether principal inverse bounds", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["uniform_principal_symbol_inverse_bound_pass_count"])}{_metric("Aether elliptic homotopies", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["principal_elliptic_homotopy_to_reference_pass_count"])}{_metric("Aether lower-order registries", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["aether"]["distributed_lower_order_coefficient_registry_complete_count"])}{_metric("G3 surplus identities", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["exact_surplus_identity_pass_count"])}{_metric("G3 surplus mismatch classes rejected", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["above_threshold_surplus_mismatch_class_reject_count"])}{_metric("G3 matched surplus controls", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["matched_surplus_necessary_control_count"])}{_metric("Registered AF York data", core["grammar_parameter_cells"]["staged_epoch"]["reviewed_future_chunk"]["family_followup"]["g3"]["registered_AF_metric_York_datum_pass_count"])}</div><p>For the sole regular Aether candidate whose finite-tilt York symbol stays elliptic, the exact determinant gap now yields a finite pointwise principal inverse bound and an elliptic homotopy to the Euclidean reference. The spatially distributed order-zero and order-one constraint coefficients, weighted relative bound, kernel estimate, and full Fredholm inverse remain unregistered. The other two regular candidates retain exact York-shell obstructions and the other 11 retain characteristic-foliation blockers. For G3, writing curvature surplus as <code>C=R3-c*v^2</code> and source surplus as <code>Y=S_beta-c*v^2</code> gives the exact Hamiltonian residual <code>C-Y</code>. Any finite point with <code>Y&gt;C</code> rejects that datum, including above-threshold mismatch controls; matched <code>C=Y</code> is only a necessary pointwise control, and no AF metric/York datum or momentum solution is registered. These are ansatz/data-class boundaries, not theory rejections.</p></section>
  {future_dossier_html}
