"""Append exactly partition 0007 to the immutable Epoch 003 formal receipt ledger."""
...
DECISION = "cumulative_formal_receipt_prefix_advanced_to_partition_0007_no_promotion"
CONFIG_REL = (
...
TEST_REL = "tests/test_continuous_scientific_pipeline_cumulative_formal_partition_0007.py"
RESULT_REL = (
    "runs/engine/continuous-scientific-pipeline-epoch-003-cumulative-formal-partition-0007/"
    "result.json"
)
...
def _load_bound_json(root: Path, binding: Mapping[str, Any]) -> dict[str, Any]:
    if set(binding) != {"path", "file_sha256", "content_sha256"}:
        raise ValueError("partition 0007 JSON binding contract mismatch")
    path = _resolve(root, str(binding["path"]))
    if _file_sha(path) != binding["file_sha256"]:
        raise ValueError("partition 0007 JSON binding file hash mismatch")
    value = _load_json(path)
    if value.get("content_sha256") != binding["content_sha256"]:
        raise ValueError("partition 0007 JSON binding content hash mismatch")
    return value
...
def _write_atomic_immutable(path: Path, value: Mapping[str, Any], maximum_bytes: int) -> None:
    ...
    if len(raw) > maximum_bytes:
        raise RuntimeError("partition 0007 artifact exceeds byte bound")
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        if path.read_bytes() != raw:
            raise ValueError(f"immutable partition 0007 artifact differs: {path.name}")
        return
    ...
    try:
        ...
        try:
            ...
            if path.read_bytes() != raw:
                raise ValueError(
                    f"immutable partition 0007 artifact differs: {path.name}"
                ) from None
    finally:
...
def load_config(root: Path, path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "continuous-scientific-pipeline-epoch-003-cumulative-formal-partition-0007"
        ...
        raise ValueError("partition 0007 config contract mismatch")
    ...
    if set(backend) != {"path", "file_sha256"}:
        raise ValueError("partition 0007 backend binding contract mismatch")
    backend_path = _resolve(root, str(backend["path"]))
    if _file_sha(backend_path) != backend["file_sha256"]:
        raise ValueError("partition 0007 backend binding mismatch")
...
def _predecessor_state(
    ...
    if (
        len(summaries) != 6
        or [row["leaf_catalog_index"] for row in summaries] != [0, 1, 2, 3, 4, 5]
        or ledger["cumulative_formal_receipt_ledger_root_sha256"]
        != predecessor["cumulative_formal_receipt_ledger_root_sha256"]
    ):
        raise ValueError("partition 0007 predecessor prefix mismatch")
...
def _selected_entry(
    ...
    index = len(summaries)
    if index != 6 or len(catalog) <= index:
        raise ValueError("partition 0007 deterministic next leaf unavailable")
    entry = catalog[index]
    prior_roots = {row["leaf_binding"]["content_sha256"] for row in summaries}
    if entry["leaf_binding"]["content_sha256"] in prior_roots:
        raise ValueError("partition 0007 selected leaf overlaps predecessor prefix")
    return entry


def _validate_preflight(value: Mapping[str, Any], config: Mapping[str, Any]) -> None:
    _validate_sealed(value, "partition 0007 preflight")
    ...
    if (
        ...
        raise ValueError("partition 0007 preflight contract mismatch")
...
def _build_preflight(root: Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not admitted:
        raise RuntimeError("partition 0007 failed resource admission")
    _validate_preflight(value, config)
...
def _validate_artifact_bindings(root: Path, bindings: Any) -> None:
    if not isinstance(bindings, list):
        raise TypeError("partition 0007 artifact bindings are not a list")
    paths = []
    for binding in bindings:
        if (
            ...
            raise ValueError("partition 0007 artifact binding mismatch")
        paths.append(binding["path"])
    if paths != sorted(set(paths)):
        raise ValueError("partition 0007 artifact binding ordering mismatch")
...
def _validate_partition(
    ...
    _validate_sealed(value, "partition 0007")
    ...
    if (
        ...
        raise ValueError("partition 0007 contract mismatch")
    ...
    for source in sorted(item for item in attempt.iterdir() if item.is_dir()):
        ...
        if target.exists():
            ...
            if existing != proposed:
                raise ValueError("immutable partition 0007 candidate artifacts differ")
            continue
...
def _build_partition(
    ...
    path = _artifact_path(root, config, "partition-0007.json")
    if path.exists():
        value = _load_json(path)
    ...
    if remaining <= 2:
        raise TimeoutError("partition 0007 exceeded total wall-clock bound")
    ...
    try:
        formal_result = _run_owned_child(
            ...
            action_name="Epoch 003 cumulative formal receipt partition 0007",
        )
...
def _derive_ledger(
    ...
    partition_binding = _binding(
        root, _artifact_path(root, config, "partition-0007.json"), partition
    )
    summary = _partition_summary(
        sequence=7,
        catalog_entry=catalog[6],
    ...
    records = [
        ...
        *({"partition_sequence": 7, **row} for row in partition["candidate_formal_receipts"]),
    ]
    ordinals = [int(row["ordinal"]) for row in records]
    if len(ordinals) != len(set(ordinals)):
        raise ValueError("partition 0007 cumulative candidate overlap")
...
def _validate_ledger(
    ...
    _validate_sealed(value, "partition 0007 cumulative ledger")
    ...
    if not isinstance(summaries, list) or not isinstance(records, list):
        raise TypeError("partition 0007 ledger rows are not lists")
    ...
    if (
        dict(value) != expected
        or summaries[:6] != predecessor_summaries
        ...
        or value["counts"]["remaining_pending_formal_receipts"] != 11_145
        ...
        raise ValueError("partition 0007 cumulative ledger contract mismatch")
...
def _derive_result(
    ...
    return {
        ...
        "execution_contract": {
            ...
            "resume": "validate_predecessor_prefix_and_reuse_immutable_partition_0007",
...
def validate_result(value: Mapping[str, Any], root: Path, config_path: Path) -> None:
    _validate_sealed(value, "partition 0007 result")
    ...
    if (
        ...
        or value["counts"]["processed_partition_prefix_length"] != 7
        or value["counts"]["cumulative_newly_processed_candidates"] != 102
        or value["counts"]["remaining_pending_formal_receipts"] != 11_145
        or value["complete_processed_partition_prefix"] is not True
        or value["complete_global_formal_receipts"] is not False
        or value["complete_comparable_evidence"] is not False
        ...
        raise ValueError("partition 0007 result contract mismatch")
