...
from sigma_theory_compiler.quartic_row1_arithmetic_expansion_campaign import (
    run_quartic_row1_arithmetic_expansion_campaign,
)
...
CONFIG = ROOT / "configs" / "backgrounds" / "quartic_row1_arithmetic_expansion_campaign.json"
ARTIFACT = RUNS / "quartic-row1-arithmetic-expansion-campaign" / "campaign.json"
...
def test_row1_all_lower_entries_and_mixed_roots_are_arithmetic_only() -> None:
    result = run_quartic_row1_arithmetic_expansion_campaign(
        _load(ROW0), _load(METRIC), _load(CONFIG)
    )
    assert result["status"] == (
        "pass_all_12_rows0_1_arithmetic_materialized_other_rows_fail_closed"
    )
    packet = result["common_row1_arithmetic_packet"]
    ...
    assert all(item["output_row"] == 1 for item in packet["lower_Jacobian_row1"])
    assert all(
        item["normalized_residual"] == "0"
        for item in packet["lower_Jacobian_row1"]
    )
    assert all(
        item["normalized_coefficient_residual"] == "0"
        for item in packet["selected_mixed_F_row1"]
    )
    assert result["counts"]["cumulative_lower_entries_normalized_per_candidate"] == 108
    for certificate in result["certificates"]:
        assert certificate["row_coverage"]["0"][
            "lower_entries_arithmetic_normalized"
        ] == 54
        assert certificate["row_coverage"]["1"][
            "lower_entries_arithmetic_normalized"
        ] == 54
...
def test_false_remainder_and_corrupt_provenance_reject() -> None:
    campaigns = (_load(ROW0), _load(METRIC))
    config = _load(CONFIG)
    false_claim = dict(config)
    false_claim["declare_component_remainder_proved"] = True
    result = run_quartic_row1_arithmetic_expansion_campaign(*campaigns, false_claim)
    assert result["status"] == "reject"
    assert "cannot be declared" in result["errors"][0]

    corrupt = json.loads(json.dumps(campaigns[0]))
    corrupt["certificates"][0]["provenance"]["row0_arithmetic_dag_sha256"] = "corrupt"
    result = run_quartic_row1_arithmetic_expansion_campaign(
