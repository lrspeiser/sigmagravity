...
def test_partition_energy_bernstein_and_loss_controls_are_exact() -> None:
    ...
    assert control["derivative_loss_negative"][
        "R3_Schwartz_Fourier_support_counterexample_encoded"
    ]
    assert control["derivative_loss_negative"][
        "low_shell_separation_margin_at_N4"
    ] == "7/32"
...
def test_all_candidates_pass_local_framework_and_fail_closed_globally() -> None:
    ...
    assert all(
        ...
        and item["shell_local_commutator_bound"][
            "per_coordinate_to_Euclidean_gradient_factor"
        ]
        == "sqrt(3)"
...
def test_incompatible_regularity_contract_or_corrupting_hash_rejects() -> None:
    r3, evolution, first_order, config = map(
        _load, (R3, EVOLUTION, FIRST_ORDER, CONFIG)
    )
    insufficient = dict(config)
    insufficient["coefficient_sobolev_order"] = 7
    result = run_quartic_dyadic_localization_campaign(
        r3, evolution, first_order, insufficient
    )
