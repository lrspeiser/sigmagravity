...
from sigma_theory_compiler.quartic_tc2_second_atom_chunk320_campaign import (
    generic_second_atom_chunk320_boundary_control,
    run_quartic_tc2_second_atom_chunk320_campaign,
)

ROOT = Path(__file__).resolve().parents[1]
RUNS = ROOT / "runs" / "physics-language"
PRIOR = RUNS / "quartic-tc2-second-atom-chunk256-campaign" / "campaign.json"
VARIABLE = RUNS / "quartic-tc2-variable-sylvester-campaign" / "campaign.json"
CONFIG = (
    ROOT
    / "configs"
    / "backgrounds"
    / "quartic_tc2_second_atom_chunk320_campaign.json"
)
ARTIFACT = RUNS / "quartic-tc2-second-atom-chunk320-campaign" / "campaign.json"
...
def test_offset_256_chain_and_generic_320_boundary_are_exact() -> None:
    chain_passed, chain = _validate_prior_chain(_load(PRIOR))
    boundary_passed, boundary = generic_second_atom_chunk320_boundary_control()
    assert chain_passed and chain["passed"] and chain["records_checked"] == 64
    assert boundary_passed and boundary["expected_next_offset"] == 320
    assert all(item["rejected"] for item in boundary["negative_controls"].values())


def test_offset_320_chunk_reaches_384_without_obstruction_but_stays_open() -> None:
    result = run_quartic_tc2_second_atom_chunk320_campaign(
        _load(PRIOR), _load(VARIABLE), _load(CONFIG)
    )
    assert result["status"] == (
        "pass_cumulative_384_second_atom_pairs_no_obstruction_remaining_fail_closed"
    )
    assert result["first_exact_obstruction"] is None
    assert result["counts"] == {
        "total_unordered_coordinate_atom_pairs": 11781,
        "prior_cumulative_evaluated_coordinate_atom_pairs": 320,
        "current_evaluated_coordinate_atom_pairs": 64,
        "cumulative_evaluated_coordinate_atom_pairs": 384,
        "remaining_unevaluated_coordinate_atom_pairs": 11397,
        "candidates": 12,
        "current_evaluated_candidate_pairs": 768,
        "current_solvable_candidate_pairs": 768,
        "current_obstructed_candidate_pairs": 0,
        "cumulative_deltaK_AB_constructions": 4608,
        "TC2_closures": 0,
        "global_H7_closures": 0,
        "lifespans_proved": 0,
    }
    assert result["exact_tensor_summary_current_chunk"] == {
        "unique_direction_pair_packets": 53,
        "nonzero_D2P55_packets": 22,
        "nonzero_D2K55_packets": 44,
        "nonzero_D2TC2_packets": 0,
        "nonzero_deltaK_AB_packets": 37,
        "maximum_deltaK_AB_rank": 6,
        "all_deltaK_AB_Hermitian": True,
        "all_second_Sylvester_residuals_zero": True,
    }
    assert result["chunk_contract"]["chunk_offset"] == 320
    assert result["pair_manifest"][0]["selector_pair_index"] == 320
    assert result["pair_manifest"][-1]["selector_pair_index"] == 383
    assert all(
        candidate["solvable"]
...
def test_prior_chain_tamper_wrong_tip_and_false_promotions_reject() -> None:
    ...
    corrupt["pair_manifest"][11]["selector_pair_index"] += 1
    _rehash(corrupt)
    result = run_quartic_tc2_second_atom_chunk320_campaign(
