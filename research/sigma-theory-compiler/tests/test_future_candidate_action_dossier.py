...
def test_proof_hierarchy_distinguishes_formula_formal_and_observation_scope() -> None:
    ...
    assert all(
        ...
        and "alpha+2k=0" in record["hierarchy_nodes"][1]["scope"]
        and "1+2k^2=0" in record["hierarchy_nodes"][1]["scope"]
        and "polyhomogeneous" in record["hierarchy_nodes"][1]["scope"]
    ...
    assert all(
        ...
        and "regular stratum F^2!=31" in record["hierarchy_nodes"][1]["scope"]
        and "rank seven and nullity two" in record["hierarchy_nodes"][1]["scope"]
...
def test_artifact_is_portable_and_secret_safe() -> None:
    ...
    assert hashlib.sha256(raw).hexdigest() == (
        "31151eb4542686c2de72bf535620677712ceb1db17e480b02683bbcddb9b602c"
    )
