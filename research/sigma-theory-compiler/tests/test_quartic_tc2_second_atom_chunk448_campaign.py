...
from sigma_theory_compiler.quartic_tc2_second_atom_chunk448_campaign import (
    generic_second_atom_chunk448_boundary_control,
    run_quartic_tc2_second_atom_chunk448_campaign,
)

ROOT = Path(__file__).resolve().parents[1]
RUNS = ROOT / "runs" / "physics-language"
PRIOR = RUNS / "quartic-tc2-second-atom-chunk384-campaign" / "campaign.json"
VARIABLE = RUNS / "quartic-tc2-variable-sylvester-campaign" / "campaign.json"
CONFIG = (
    ROOT
    / "configs"
    / "backgrounds"
    / "quartic_tc2_second_atom_chunk448_campaign.json"
)
ARTIFACT = RUNS / "quartic-tc2-second-atom-chunk448-campaign" / "campaign.json"
...
def test_offset_384_chain_and_448_boundary_are_exact() -> None:
    chain_passed, chain = _validate_prior_chain(_load(PRIOR))
    boundary_passed, boundary = generic_second_atom_chunk448_boundary_control()
    assert chain_passed and chain["passed"] and chain["records_checked"] == 64
    assert boundary_passed and boundary["expected_next_offset"] == 448
    assert all(item["rejected"] for item in boundary["negative_controls"].values())


def test_offset_448_chunk_reaches_512_without_obstruction_but_stays_open() -> None:
    result = run_quartic_tc2_second_atom_chunk448_campaign(
    ...
    assert result["counts"] == {
        ...
        "prior_cumulative_evaluated_coordinate_atom_pairs": 448,
        "current_evaluated_coordinate_atom_pairs": 64,
        "cumulative_evaluated_coordinate_atom_pairs": 512,
        "remaining_unevaluated_coordinate_atom_pairs": 11269,
        "candidates": 12,
        "current_evaluated_candidate_pairs": 768,
        "current_solvable_candidate_pairs": 768,
        "current_obstructed_candidate_pairs": 0,
        "cumulative_deltaK_AB_constructions": 6144,
        "TC2_closures": 0,
    ...
    assert result["exact_tensor_summary_current_chunk"] == {
        ...
        "nonzero_D2P55_packets": 49,
        "nonzero_D2K55_packets": 53,
        "nonzero_D2TC2_packets": 0,
        "nonzero_deltaK_AB_packets": 51,
        "maximum_deltaK_AB_rank": 6,
        "all_deltaK_AB_Hermitian": True,
        "all_second_Sylvester_residuals_zero": True,
    }
    assert result["chunk_contract"]["chunk_offset"] == 448
    assert result["pair_manifest"][0]["selector_pair_index"] == 448
    assert result["pair_manifest"][-1]["selector_pair_index"] == 511
    assert all(
        candidate["solvable"]
