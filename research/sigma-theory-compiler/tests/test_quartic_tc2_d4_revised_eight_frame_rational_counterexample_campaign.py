...
from sigma_theory_compiler.quartic_tc2_d4_revised_eight_frame_rational_counterexample_campaign import (
    EXPECTED_EXACT_GATE_SHA256,
    FALSE_CLAIMS,
    TRUE_CLAIMS,
    RevisedEightFrameRationalCounterexampleError,
    validate_campaign,
)
...
def test_next_low_height_fail_fast_point(artifact: dict) -> None:
...
def test_bounded_preserving_envelope_classification(artifact: dict) -> None:
    ...
    assert envelope["support_search_maximum"] == 7
    assert envelope["sparsest_support_size"] <= 7
    assert envelope["sparsest_support_feasible_envelopes"] > 0
    assert envelope["zero_at_all_eight_predecessor_frames"] is True


def test_bounded_local_escape_closes_only_new_frame(artifact: dict) -> None:
    bounded = artifact["exact_gate"]["bounded_next_escape"]
    exact_range = bounded["exact_range_classification"]
    ...
    assert completion["prior_eight_direction_certificates_preserved"] is True
    assert completion["total_local_direction_certificates"] == 9
...
def test_resealed_semantic_tamper_rejected(
    ...
    with pytest.raises(RevisedEightFrameRationalCounterexampleError):
