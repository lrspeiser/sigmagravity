...
from sigma_theory_compiler.quartic_tc2_second_atom_chunk256_campaign import (
    generic_second_atom_chunk256_boundary_control,
    run_quartic_tc2_second_atom_chunk256_campaign,
)

ROOT = Path(__file__).resolve().parents[1]
RUNS = ROOT / "runs" / "physics-language"
PRIOR = RUNS / "quartic-tc2-second-atom-chunk192-campaign" / "campaign.json"
VARIABLE = RUNS / "quartic-tc2-variable-sylvester-campaign" / "campaign.json"
CONFIG = (
    ROOT
    / "configs"
    / "backgrounds"
    / "quartic_tc2_second_atom_chunk256_campaign.json"
)
ARTIFACT = RUNS / "quartic-tc2-second-atom-chunk256-campaign" / "campaign.json"
...
def test_offset_192_chain_and_256_boundary_controls_are_exact() -> None:
    ...
    assert boundary_passed and boundary["expected_next_offset"] == 256
    assert all(item["rejected"] for item in boundary["negative_controls"].values())


def test_offset_256_chunk_reaches_320_without_obstruction_but_stays_open() -> None:
    result = run_quartic_tc2_second_atom_chunk256_campaign(
    ...
    assert result["counts"] == {
        ...
        "prior_cumulative_evaluated_coordinate_atom_pairs": 256,
        "current_evaluated_coordinate_atom_pairs": 64,
        "cumulative_evaluated_coordinate_atom_pairs": 320,
        "remaining_unevaluated_coordinate_atom_pairs": 11461,
        "candidates": 12,
        "current_evaluated_candidate_pairs": 768,
        "current_solvable_candidate_pairs": 768,
        "current_obstructed_candidate_pairs": 0,
        "cumulative_deltaK_AB_constructions": 3840,
        "TC2_closures": 0,
    ...
    assert result["chunk_contract"]["chunk_offset"] == 256
    assert result["pair_manifest"][0]["selector_pair_index"] == 256
    assert result["pair_manifest"][-1]["selector_pair_index"] == 319
    assert all(
        candidate["solvable"]
...
def test_prior_chain_tamper_wrong_tip_and_false_promotions_reject() -> None:
    ...
    corrupt["pair_manifest"][9]["selector_pair_index"] += 1
    _rehash(corrupt)
    result = run_quartic_tc2_second_atom_chunk256_campaign(
