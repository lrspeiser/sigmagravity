...
from sigma_theory_compiler.quartic_row3_arithmetic_expansion_campaign import (
    run_quartic_row3_arithmetic_expansion_campaign,
)

ROOT = Path(__file__).resolve().parents[1]
RUNS = ROOT / "runs" / "physics-language"
ROW2 = RUNS / "quartic-row2-arithmetic-expansion-campaign" / "campaign.json"
METRIC = RUNS / "quartic-metric-rows-tensor-dag-campaign" / "campaign.json"
CONFIG = ROOT / "configs" / "backgrounds" / "quartic_row3_arithmetic_expansion_campaign.json"
ARTIFACT = RUNS / "quartic-row3-arithmetic-expansion-campaign" / "campaign.json"
...
def test_row3_exact_current_and_cumulative_coverage() -> None:
    result = run_quartic_row3_arithmetic_expansion_campaign(
        _load(ROW2), _load(METRIC), _load(CONFIG)
    )
    assert result["status"] == (
        "pass_all_12_rows0_3_arithmetic_materialized_other_rows_fail_closed"
    )
    packet = result["common_row3_arithmetic_packet"]
    ...
    assert all(item["output_row"] == 3 for item in packet["lower_Jacobian"])
    ...
    assert result["counts"]["cumulative_lower_entries_normalized_per_candidate"] == 216
    assert result["counts"]["cumulative_selected_mixed_entries_per_candidate"] == 24
...
def test_false_remainder_and_corrupt_provenance_reject() -> None:
    campaigns = (_load(ROW2), _load(METRIC))
    config = _load(CONFIG)
    false_claim = dict(config)
    false_claim["declare_component_remainder_proved"] = True
    result = run_quartic_row3_arithmetic_expansion_campaign(*campaigns, false_claim)
    assert result["status"] == "reject"
    assert "cannot be declared" in result["errors"][0]

    corrupt = json.loads(json.dumps(campaigns[0]))
    corrupt["certificates"][0]["provenance"]["row2_arithmetic_dag_sha256"] = "corrupt"
    result = run_quartic_row3_arithmetic_expansion_campaign(
