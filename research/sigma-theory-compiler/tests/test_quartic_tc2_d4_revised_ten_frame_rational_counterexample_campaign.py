...
from sigma_theory_compiler.quartic_tc2_d4_revised_ten_frame_rational_counterexample_campaign import (
    EXPECTED_EXACT_GATE_SHA256,
    FALSE_CLAIMS,
    TRUE_CLAIMS,
    RevisedTenFrameRationalCounterexampleError,
    validate_campaign,
)
...
def test_next_signed_height_one_point(artifact: dict) -> None:
    ...
    assert gate["search_protocol"]["remaining_bounded_selector_points_after_obstruction"] == 1
...
def test_bounded_envelope_and_local_repair(artifact: dict) -> None:
    ...
    assert envelope["support_search_maximum"] == 12
    assert envelope["sparsest_support_size"] <= 12
    assert envelope["sparsest_support_feasible_envelopes"] > 0
    assert envelope["zero_at_all_ten_predecessor_frames"] is True
    ...
    assert completion["prior_ten_direction_certificates_preserved"] is True
    assert completion["total_local_direction_certificates"] == 11
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            (
                "exact_gate", "bounded_next_escape", "local_completion",
                "candidate_records", 0, "corrected_residual_sha256",
            ),
            "0" * 64,
        ),
...
def test_resealed_semantic_tamper_rejected(
    ...
    with pytest.raises(RevisedTenFrameRationalCounterexampleError):
