...
import sigma_theory_compiler.continuous_scientific_pipeline_cumulative_formal_partition_0006 as worker
from sigma_theory_compiler.continuous_scientific_pipeline_cumulative_formal_partition_0006 import (
    CONFIG_REL,
    RESULT_REL,
    _leaf_catalog,
    _load_bound_json,
...
def test_checked_partition_0006_extends_exact_prefix_without_promotion() -> None:
    result, ledger, partition = _checked()
    validate_result(result, ROOT, CONFIG)
    assert result["decision"] == (
        "cumulative_formal_receipt_prefix_advanced_to_partition_0006_no_promotion"
    )
    assert result["counts"] == {
        "processed_partition_prefix_length": 6,
        "processed_leaf_pages": 6,
        "cumulative_formally_checked_candidates": 84,
        "cumulative_newly_processed_candidates": 82,
        "cumulative_reconciled_preserved_candidates": 2,
        "cumulative_candidate_rejects": 84,
        ...
        "remaining_pending_formal_receipts": 11_165,
        "rank_assignments": 0,
        "candidate_promotions": 0,
    }
    assert partition["counts"]["partition_candidates"] == 2
    assert partition["counts"]["newly_processed_candidates"] == 2
    assert partition["counts"]["candidate_rejects"] == 2
...
def test_partition_0006_leaf_is_deterministic_and_merkle_reachable() -> None:
    ...
    assert entry["catalog_index"] == 5
    assert entry["leaf_binding"]["path"].endswith("node-1004905984-1004908032.json")
    assert entry["pending_candidate_count"] == 2
    assert entry["leaf_queue_root_sha256"] == (
        "e57eaa93798fd8d672793d8d661682b899043cb5925844d94d7af4e904bbd512"
    )
    assert len(entry["hierarchy_path"]) == 6
    leaf = _load_bound_json(ROOT, entry["leaf_binding"])
    assert leaf["ordinal_interval"] == {
        "candidate_count": 2048,
        "start_ordinal": 1_004_905_984,
        "end_ordinal_exclusive": 1_004_908_032,
    }
...
def test_predecessor_ledger_is_preserved_verbatim() -> None:
    ...
    assert ledger["processed_partition_summaries"][:5] == previous["processed_partition_summaries"]
    assert (
        ledger["cumulative_formal_receipt_records"][:82]
...
def test_selected_leaf_overlap_is_rejected() -> None:
    ...
    transplanted[5]["leaf_binding"] = summaries[0]["leaf_binding"]
...
def test_three_partition_prefix_rejects_overlap_or_gap(mutation: str) -> None:
    ...
    if mutation == "overlap":
        summaries[5]["leaf_catalog_index"] = 4
        summaries[5]["leaf_binding"] = summaries[4]["leaf_binding"]
    else:
        summaries[5]["leaf_catalog_index"] = 6
        summaries[5]["leaf_binding"] = catalog[6]["leaf_binding"]
    partitions = [
...
def test_completed_partition_0006_resumes_without_child_execution(
...
def test_validation_never_opens_runtime_gpu_live_sqlite_or_supervisor(
    ...
    def guarded_text(path: Path, *args: object, **kwargs: object) -> str:
        if forbidden(path):
            raise AssertionError("partition 0006 validator opened excluded mutable state")
