...
import sigma_theory_compiler.continuous_scientific_pipeline_cumulative_formal_partition_0005 as worker
from sigma_theory_compiler.continuous_scientific_pipeline_cumulative_formal_partition_0005 import (
    CONFIG_REL,
    RESULT_REL,
    _leaf_catalog,
    _load_bound_json,
...
def test_checked_partition_0005_extends_exact_prefix_without_promotion() -> None:
...
def test_partition_0005_leaf_is_deterministic_and_merkle_reachable() -> None:
    ...
    assert entry["catalog_index"] == 4
    assert entry["leaf_binding"]["path"].endswith("node-1004897792-1004905984.json")
    assert entry["pending_candidate_count"] == 2
    assert entry["leaf_queue_root_sha256"] == (
        "75a97b0f95d0c50be5ac30fbedab12d4caf7a5f22fd140c8a517eb3d97ff308d"
    )
    assert len(entry["hierarchy_path"]) == 4
    leaf = _load_bound_json(ROOT, entry["leaf_binding"])
    assert leaf["ordinal_interval"] == {
        "candidate_count": 8192,
        "start_ordinal": 1_004_897_792,
        "end_ordinal_exclusive": 1_004_905_984,
    }
...
def test_predecessor_ledger_is_preserved_verbatim() -> None:
    ...
    assert ledger["processed_partition_summaries"][:4] == previous["processed_partition_summaries"]
    assert (
        ledger["cumulative_formal_receipt_records"][:80]
...
def test_selected_leaf_overlap_is_rejected() -> None:
    ...
    transplanted[4]["leaf_binding"] = summaries[0]["leaf_binding"]
...
def test_three_partition_prefix_rejects_overlap_or_gap(mutation: str) -> None:
    ...
    if mutation == "overlap":
        summaries[4]["leaf_catalog_index"] = 3
        summaries[4]["leaf_binding"] = summaries[3]["leaf_binding"]
    else:
        summaries[4]["leaf_catalog_index"] = 5
        summaries[4]["leaf_binding"] = catalog[5]["leaf_binding"]
    partitions = [
...
def test_completed_partition_0005_resumes_without_child_execution(
...
def test_validation_never_opens_runtime_gpu_live_sqlite_or_supervisor(
    ...
    def guarded_text(path: Path, *args: object, **kwargs: object) -> str:
        if forbidden(path):
            raise AssertionError("partition 0005 validator opened excluded mutable state")
