...
import sigma_theory_compiler.continuous_scientific_pipeline_cumulative_formal_partition_0004 as worker
from sigma_theory_compiler.continuous_scientific_pipeline_cumulative_formal_partition_0004 import (
    CONFIG_REL,
    RESULT_REL,
    _leaf_catalog,
    _load_bound_json,
...
def test_checked_partition_0004_extends_exact_prefix_without_promotion() -> None:
    ...
    assert partition["counts"]["partition_candidates"] == 15
    assert partition["counts"]["newly_processed_candidates"] == 15
    assert partition["counts"]["candidate_rejects"] == 15
...
def test_partition_0004_leaf_is_deterministic_and_merkle_reachable() -> None:
    ...
    assert entry["catalog_index"] == 3
    assert entry["leaf_binding"]["path"].endswith("node-1004882432-1004883456.json")
    assert entry["pending_candidate_count"] == 15
    assert entry["leaf_queue_root_sha256"] == (
        "78cac8734b00f0fbc186380a785ab7242044e3f13fbd4bf4d7cd699fb98079ec"
    )
    ...
    assert leaf["ordinal_interval"] == {
        ...
        "start_ordinal": 1_004_882_432,
        "end_ordinal_exclusive": 1_004_883_456,
    }
...
def test_predecessor_ledger_is_preserved_verbatim() -> None:
    ...
    assert ledger["processed_partition_summaries"][:3] == previous["processed_partition_summaries"]
    assert (
        ledger["cumulative_formal_receipt_records"][:65]
...
def test_selected_leaf_overlap_is_rejected() -> None:
    ...
    transplanted[3]["leaf_binding"] = summaries[0]["leaf_binding"]
...
def test_three_partition_prefix_rejects_overlap_or_gap(mutation: str) -> None:
    ...
    if mutation == "overlap":
        summaries[3]["leaf_catalog_index"] = 2
        summaries[3]["leaf_binding"] = summaries[2]["leaf_binding"]
    else:
        summaries[3]["leaf_catalog_index"] = 4
        summaries[3]["leaf_binding"] = catalog[4]["leaf_binding"]
    partitions = [
...
def test_completed_partition_0004_resumes_without_child_execution(
...
def test_validation_never_opens_runtime_gpu_live_sqlite_or_supervisor(
    ...
    def guarded_text(path: Path, *args: object, **kwargs: object) -> str:
        if forbidden(path):
            raise AssertionError("partition 0004 validator opened excluded mutable state")
