...
from sigma_theory_compiler.quartic_tc2_second_atom_chunk192_campaign import (
    generic_second_atom_chunk192_boundary_control,
    run_quartic_tc2_second_atom_chunk192_campaign,
)

ROOT = Path(__file__).resolve().parents[1]
RUNS = ROOT / "runs" / "physics-language"
PRIOR = RUNS / "quartic-tc2-second-atom-chunk128-campaign" / "campaign.json"
VARIABLE = RUNS / "quartic-tc2-variable-sylvester-campaign" / "campaign.json"
CONFIG = (
    ROOT
    / "configs"
    / "backgrounds"
    / "quartic_tc2_second_atom_chunk192_campaign.json"
)
ARTIFACT = RUNS / "quartic-tc2-second-atom-chunk192-campaign" / "campaign.json"
...
def test_offset_128_chain_and_192_boundary_controls_are_exact() -> None:
    ...
    assert boundary_passed and boundary["expected_next_offset"] == 192
    assert all(item["rejected"] for item in boundary["negative_controls"].values())


def test_offset_192_chunk_reaches_256_without_obstruction_but_stays_open() -> None:
    result = run_quartic_tc2_second_atom_chunk192_campaign(
    ...
    assert result["counts"] == {
        ...
        "prior_cumulative_evaluated_coordinate_atom_pairs": 192,
        "current_evaluated_coordinate_atom_pairs": 64,
        "cumulative_evaluated_coordinate_atom_pairs": 256,
        "remaining_unevaluated_coordinate_atom_pairs": 11525,
        "candidates": 12,
        "current_evaluated_candidate_pairs": 768,
        "current_solvable_candidate_pairs": 768,
        "current_obstructed_candidate_pairs": 0,
        "cumulative_deltaK_AB_constructions": 3072,
        "TC2_closures": 0,
        "global_H7_closures": 0,
        "lifespans_proved": 0,
    }
    assert result["exact_tensor_summary_current_chunk"] == {
        "unique_direction_pair_packets": 50,
        "nonzero_D2P55_packets": 19,
        "nonzero_D2K55_packets": 41,
    ...
    assert contract["chunk_offset"] == 192
    assert contract["evaluated_chunk_size"] == 64
    assert not contract["stopped_at_first_obstruction"]
    assert result["pair_manifest"][0]["selector_pair_index"] == 192
    assert result["pair_manifest"][-1]["selector_pair_index"] == 255
    assert all(
        candidate["solvable"]
...
def test_prior_chain_tamper_wrong_tip_and_false_promotions_reject() -> None:
    ...
    corrupt["pair_manifest"][7]["selector_pair_index"] += 1
    _rehash(corrupt)
    result = run_quartic_tc2_second_atom_chunk192_campaign(
