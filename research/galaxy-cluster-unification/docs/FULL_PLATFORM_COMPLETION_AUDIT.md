...
| Safe formula operators | Partial | Grad/div/curl/Laplacian, products, gates, coupled scalar elliptic fields, LOS/path operations, Cartesian convolution, multiple potentials, and consistent periodic spectral derivatives for FFT jobs | General tensor algebra, cylindrical nonlocal kernels, broader nonlinear constitutive forms |
| Isolated advanced-code plug-ins | Partial | Ed25519 manifest identity, trusted-publisher separation, exact file rehash, pinned Python ABI, production image-digest gate, and a real single-use no-network/read-only/non-root/quota container fixture; public preflight never executes code and production jobs fail closed | Project publisher enrollment/revocation, safe archive upload/extraction, private package registry, dedicated sandbox host, image signing/scanning, binary artifact ABI, malware/abuse controls, and hosted lifecycle; never execute plug-ins in Vercel or the safe worker |
| Generic Cartesian 2D/3D worker | Partial | Finite-volume scalar elliptic, nonlinear Picard/Anderson/Newton–Krylov, coupled fields, diagnostics, nonlocal Cartesian fixture, and direct periodic FFT Poisson in 2D/3D with explicit zero-mode/gauge, conservation, energy, anisotropic-grid and resolution acceptance | Vector/tensor solves, mixed/Neumann execution, nonuniform periodic grids, refinement and production scaling |
...
| Formula-independence acceptance suite | Partial | Newtonian, AQUAL-like, QUMOND, Refracted Gravity, nonlocal, two-potential, nonlinear and axisymmetric fixtures; the signed isolated external Python fixture reproduces fixed MOND while checking the container boundary | Broader vector/tensor fixtures, a field-solver plug-in rather than only algebraic acceleration, and manifests submitted by external researchers without application changes |
