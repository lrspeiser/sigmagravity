...
### Frozen Chandra response image result

The Chandra extended-source response image passed its terminal gate before any
XRISM energy distribution was summarized or fit. CIAO `merge_obs` combined
ObsIDs 3231 and 15187 at 0.492-arcsec sampling over 0.5--7.0 keV and returned
exit code zero. The frozen 12-arcmin crop is 1463 by 1463 pixels, has 486,683
positive pixels, is centered at RA 290.2989764 deg and Dec 43.9345609 deg, and
has SHA-256
`4db29ad4f17fb05cbf264865709ed604a2e3c9976fac9bd749923210c2c99bc9`.

Two infrastructure failures are retained in the machine-readable report. The
first run stopped before opening either event table because `PFILES` was unset.
The second run's outer Windows runner ended after 120 seconds while CIAO
continued, but its temporary files were automatically removed after completion
and were not accepted as evidence. The clean third run used the identical
frozen command and WSL-native temporary storage, returned exit code zero, and
passed the WCS, width, finite-pixel, positivity, and hash checks. The CIAO
warning against using the merged event list for Chandra ARF/RMF construction is
not crossed: only the exposure-corrected flux image is used as the XRISM
extended-source sky template. The report SHA-256 is
`41952d132764903e327e7c243f454a57f930176281aa1cf111b243eb59b760d2`.

No XRISM spectrum, NXB, RMF, exposure map, ARF, velocity, validation target, or
holdout target was opened or generated by this stage.

Before response generation, an audit of the installed HEASoft 6.36 parameter
files exposed a hidden `rslmkrmf` `time` default of
`2019-01-01T00:00:00`. Protocol 1.0.2 therefore explicitly passes the exact
`DATE-OBS` value from each corrected branch event to select CALDB response
files. It also hashes the `xrtraytrace`, `heasim`, `xaxmaarfgen`, and
`aharfgen` executables called transitively by `xaarfgen`, and freezes the image
source coordinate and branch-scoped raytrace reuse rule. This amendment changes
no scientific input or prepared GTI, region, or Chandra-image byte and was
