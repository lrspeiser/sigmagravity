...
| Cluster source dynamics | The response-aware A2319 run completed all seven NXB prefits and 21 source fits. All seven primary velocities converged and six regions passed both robustness branches, but the terminal gate failed because the public regional NXB model fit poorly and pinned nuisance normalizations to hard bounds. | Time-odd source not admitted; A3667 validation and A754 holdout remain sealed pending an independently specified regional NXB likelihood or a different direct-velocity evidence route. |
