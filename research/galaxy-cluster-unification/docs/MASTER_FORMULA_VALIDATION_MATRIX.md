...
| Cluster source dynamics | The response-aware A2319 run completed all seven NXB prefits and 21 source fits, but its public regional NXB model failed. V19CZ then preserved every additional shape freedom required by the independent official recipe; six regions still had reduced chi-square 10.97--17.96. The replacement V19DA Chandra preflight passes across all 5,082 cells and 20,328 archived products. V19DB has now combined all 43 Bullet primary regions from all 3,483 primary cells with exact count conservation; its worst direct-versus-hierarchical forward-fold difference is `5.65e-9`. | Time-odd source not admitted. Response-aware Bullet spectral fitting is authorized but not yet run. The public Resolve-v1 small-subarray route is retired. Abell 2146 is internally sealed but nonpristine transfer. A3667 validation and A754 holdout remain sealed. |
