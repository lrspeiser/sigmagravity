...
| Cluster source dynamics | The response-aware A2319 run completed all seven NXB prefits and 21 source fits, but its public regional NXB model failed. V19CZ then preserved every additional shape freedom required by the independent official recipe in seven NXB-only retries; six regions still had reduced chi-square 10.97--17.96 and every region pinned free parameters to bounds. | Time-odd source not admitted; the public Resolve-v1 small-subarray route is retired. A3667 validation and A754 holdout remain sealed pending a newly released region-aware likelihood or a different direct-velocity observable. |
