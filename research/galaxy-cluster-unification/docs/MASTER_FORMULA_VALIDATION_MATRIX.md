...
| Cluster source dynamics | The response-aware A2319 route failed because the public regional NXB model was unacceptable even with every official-state shape freedom. The replacement V19DA Chandra chain passed exact combination, response, gain and weight-transport gates. V19DE2 then repaired the missing AtomDB binding and completed all 404 integrated APEC/MEKAL profile points with finite two-start fits. Six of seven profile gates pass, but APEC has a distinct `z=0.3050` secondary minimum only `Delta WStat=1.7802` above the coarse global minimum, inside the frozen `6.63` rejection threshold. | Time-odd source not admitted. The exact integrated two-temperature Chandra closure is a scientific failure and may not be retuned. No posterior-predictive/Sobol or regional stage is authorized; Abell 2146, A3667 validation and A754 holdout remain sealed. Seek a materially different direct measurement such as a region-aware calorimeter likelihood or resolved kSZ rather than another fit adjustment. |
