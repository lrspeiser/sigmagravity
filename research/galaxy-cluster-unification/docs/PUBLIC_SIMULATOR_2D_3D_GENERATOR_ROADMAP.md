...
| Asynchronous multi-system batch API | one frozen manifest and policy run across generated/uploaded 2D or 3D bundles, with polling, cancellation, restart recovery, deterministic rehashed reports, and P0733 composition of reusable field children with separately cached observation children | local reference service only; durable workers are not connected and photon lensing is not implemented |
