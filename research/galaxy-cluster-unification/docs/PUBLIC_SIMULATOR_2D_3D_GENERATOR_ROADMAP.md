...
| Asynchronous multi-system batch API | one frozen manifest and policy run across generated/uploaded 2D or 3D bundles, with polling, cancellation, restart recovery, deterministic rehashed reports, P0733 composition of reusable field children with separately cached observation children, and P0734 channel-separated photon maps | local reference service only; durable workers and raw multiple-image lensing are not connected |
...
| 7. 2D-to-3D reconstruction | The v0.22 local worker saves every seeded 2D/3D prior draw, including declared mass, scale, structure, distance, thin-map inclination, warp, thickness, and co-spatial unseen-baryon uncertainty, with exact projection and percentile maps. Add bulge deprojection, PSF/beam/noise/mask likelihoods, and posterior weights rather than prior-only draws. | Synthetic recovery tests cover known thickness/bulge/inclination; real results report the spread across data-conditioned reconstructions. |
