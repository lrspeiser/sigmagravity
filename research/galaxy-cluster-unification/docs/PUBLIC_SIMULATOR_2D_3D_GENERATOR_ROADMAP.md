...
| 7. 2D-to-3D reconstruction | The v0.22 worker saves every seeded 2D/3D prior draw with exact projection and percentile maps; v0.23 propagates one confirmed field model and shared observations across selected draws and reports unweighted prediction quantiles. Add bulge deprojection, PSF/beam/noise/mask likelihoods, posterior weights, and weighted prediction aggregation. | Synthetic recovery tests cover known thickness/bulge/inclination; real results report calibrated spread across data-conditioned reconstructions. |
