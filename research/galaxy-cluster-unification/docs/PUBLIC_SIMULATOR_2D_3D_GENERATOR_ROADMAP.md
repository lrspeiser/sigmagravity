...
| Content-addressed field job | verified array bundle, deterministic job/scientific hashes, residual history, output hashes, resource log, artifact index, and CLI pass end to end; private content-addressed Vercel objects pass live acceptance; transactional Postgres/outbox and private queue consumers pass duplicate/lease/cancellation/finalization tests | Neon terms must be accepted before the job database can be provisioned; the stateless hosted worker is not connected |
...
| Asynchronous reference API | immutable NPZ upload, queue, polling, events, cancellation, restart recovery, and rehashed artifact downloads pass through real HTTP and the Python worker; durable private objects, Vercel Queue triggers, Postgres state machine/outbox, and stateless hash-reference handoff are implemented separately | the Postgres resource and stateless container are not deployed; public researcher jobs remain disabled |
...
| 15. Storage and reproducibility | Private content-addressed objects and the nine-table Postgres schema/outbox are built. Provision and migrate Neon, wire model/dataset/code/solver/container/seed/grid/boundary/log/prediction/plot identities, and sign citation-ready manifests. | A clean worker can reproduce selected run hashes/tolerances from only the manifest and permitted inputs; failed runs retain useful artifacts. |
| 16. Hosting and operations | The non-root Python image, private queue triggers, stateless handoff, leases, cancellation precedence and finalization contracts are built. Deploy CPU resource classes, connect them to Postgres/Blob/Queue, and add auth, quotas, monitoring, cost controls, backups, abuse handling, and uptime/support policy. | Public users can submit a bounded batch, follow status, download artifacts, and cannot exhaust shared resources or access sealed data. |
