...
def directional_error(
    ...
    gradient_local, gradient_memory = bounded_homology_gradients(local, memory)
    step = 1e-6 * max(1.0, np.linalg.norm(local), np.linalg.norm(memory))
    finite = float(
        (
            bounded_homology_potential(
...
def algebra_audit() -> dict[str, float]:
    ...
    for _ in range(256):
        ...
        value = float(bounded_homology_potential(local, memory))
        minimum_potential = min(minimum_potential, value)
        maximum_potential = max(maximum_potential, value)
        minimum_gram = min(minimum_gram, float(homology_invariants(local, memory)[3]))
    ...
    proportional = 2.3 * local_diagonal
    proportional_value = abs(
        float(
            bounded_homology_potential(
                rotation @ local_diagonal @ rotation.T,
                rotation @ proportional @ rotation.T,
            )
        )
    )
    nonproportional = np.diag([0.2, 0.5, -0.7])
    nonproportional_value = float(bounded_homology_potential(local_diagonal, nonproportional))
    weak_scale = 1e-3
    gram = float(homology_invariants(local_diagonal, nonproportional)[3])
    leading = weak_scale**4 * gram
    exact = float(
        bounded_homology_potential(
            weak_scale * local_diagonal,
            weak_scale * nonproportional,
        )
    )
    return {
        "minimum_random_potential": minimum_potential,
        "maximum_random_potential": maximum_potential,
        "minimum_random_gram_determinant": minimum_gram,
        ...
        "proportional_absolute_potential": proportional_value,
        "fixed_nonproportional_potential": nonproportional_value,
        "quartic_onset_relative_error": abs(exact / leading - 1.0),
    ...
    return (
        {
            ...
            "minimum_gram_determinant": float(np.min(field.gram_determinant)),
...
def plot_results(
    ...
    alternative = np.diag([0.2, 0.5, -0.7])
    mixtures = np.linspace(0.0, 1.0, 300)
    values = [
        bounded_homology_potential(local, (1.0 - mix) * 2.3 * local + mix * alternative)
        for mix in mixtures
    ]
    axes[0, 1].plot(mixtures, values, color="#8a3ffc")
    axes[0, 1].set_xlabel("departure from proportional scale homology")
    axes[0, 1].set_ylabel(r"$\mathcal{V}_{\rm hom}$")
    axes[0, 1].set_title("Proportional null and shape activation")
    ...
    figure.suptitle("Sigma v3F preregistered scale-homology audit", fontsize=14)
    figure.savefig(output / "scale_homology_audit.png", dpi=180)
    plt.close(figure)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the frozen Sigma v3F structural audit.")
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs" / "sigma_v3f_scale_homology_action_audit.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "results" / "sigma_v3f_scale_homology_action_audit",
    )
    ...
    gates = {
        ...
        "proportional_null": bool(
            algebra["proportional_absolute_potential"]
            <= float(thresholds["maximum_proportional_absolute_potential"])
        ),
        "nonproportional_activation": bool(
            algebra["fixed_nonproportional_potential"]
            >= float(thresholds["minimum_nonproportional_potential"])
        ),
        "quartic_onset": bool(
    ...
    report = {
        ...
        "decision": (
            ...
            if all_pass
            else "retire_v3f_homology_as_frozen_structural_mechanism"
        ),
