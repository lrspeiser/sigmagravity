#!/usr/bin/env python3
"""Test self-adjoint field-line diffusion on spent raw lensing."""
...
DEFAULT_CONFIG = ROOT / "configs" / "p0657_field_line_diffusion.json"
...
def main():
    ...
    if protocol.get("status") != "frozen_before_any_P0657_score":
        raise RuntimeError("P0657 protocol is not frozen")

    inputs = protocol["inputs"]
    p0644 = read_json(ROOT / inputs["P0644_protocol"])
    p0645_protocol = read_json(ROOT / inputs["P0645_protocol"])
    p0645_report = read_json(ROOT / inputs["P0645_report"])
    p0648_report = read_json(ROOT / inputs["P0648_report"])
    p0651_report = read_json(ROOT / inputs["P0651_report"])
    p0652_report = read_json(ROOT / inputs["P0652_report"])
    p0654_report = read_json(ROOT / inputs["P0654_report"])
    p0655_report = read_json(ROOT / inputs["P0655_report"])
    p0656_report = read_json(ROOT / inputs["P0656_report"])
    if not all(p0651_report["map_gate_results"].values()):
        raise RuntimeError("registered map gates no longer pass")
    if p0656_report["candidate_selected_or_advanced"]:
        raise RuntimeError("descriptive P0656 illegally selected a candidate")
    ...
    if not np.isclose(actual_padding, expected_padding):
        raise RuntimeError("P0657 padding is not exactly the inherited trace cap")
    ...
    cv_summary, fold_rows, prediction_frames = evaluate_candidate(
        ...
        stage="exact_field_line_diffusion",
    )
    ...
    gate_results = {
        ...
        "transport_flux_conserved": float(audit["transport_flux_sum_relative_error"])
        <= gates["transport_flux_sum_relative_error_max"],
        "transport_no_overshoot": float(audit["transport_component_overshoot_fraction"])
        <= gates["transport_component_overshoot_fraction_max"],
        "transport_declared_conservative": bool(audit["transport_is_source_conservative"])
        is bool(gates["transport_is_source_conservative"]),
        "transport_self_adjoint": bool(audit["transport_is_self_adjoint_diffusion"])
        is bool(gates["transport_is_self_adjoint_diffusion"]),
    ...
    report = {
        "report_version": "P0657-FIELD-LINE-DIFFUSION-RESULTS-1.0.0",
        ...
        "comparison": {
            ...
            "P0652_finite_gather_CV_RMS_arcsec": float(
                p0652_report["comparison"]["primary_CV_RMS_arcsec"]
            ),
            "P0654_padded_gather_CV_RMS_arcsec": float(
                p0654_report["comparison"]["candidate_CV_RMS_arcsec"]
            ),
            "P0655_deposit_CV_RMS_arcsec": float(
                p0655_report["comparison"]["candidate_CV_RMS_arcsec"]
            ),
    ...
    labels = ["lambda 0", "matched m=3", "P0652 gather", "P0657 diffusion"]
    values = [
        baseline_cv,
        multipole_cv,
        p0652_report["comparison"]["primary_CV_RMS_arcsec"],
    ...
    axes[0].set(ylabel="pooled exact CV RMS (arcsec)", title="Self-adjoint transport")
    ...
    axes[1].set(
        ...
        title="Field-line diffusion fold stability",
    )
    figure.tight_layout()
    figure.savefig(output / "field_line_diffusion.png", dpi=180)
    plt.close(figure)
    summary = f"""# P0657 field-line diffusion
...
- Vector-flux sum error: **{audit['transport_flux_sum_relative_error']:.3e}**.
- Component overshoot: **{audit['transport_component_overshoot_fraction']:.3e}**.
