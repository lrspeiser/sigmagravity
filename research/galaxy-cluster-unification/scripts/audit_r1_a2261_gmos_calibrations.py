#!/usr/bin/env python3
"""Audit frozen A2261 GMOS calibrations before science processing."""
...
CAL = ROOT / "data/derived/r1_a2261_gmos_reconstruction/calibrations"
REPORT = ROOT / "results/r1_a2261_gmos_calibrations/report.json"
...
def audit_biases(config: dict) -> tuple[list[dict], bool, bool]:
    ...
    construction = all(row["input_count"] >= limits["minimum_inputs_per_night_master_bias"] and row["sigma_clipped_median_recorded"] for row in rows) and all(header_crosschecks)
...
def main() -> None:
    ...
    report = {
        "report_version": "R1B2-A2261-GMOS-calibrations-0.1",
        ...
        "authorization": {"process_four_science_frames_with_frozen_mapping": gate, "fit_stellar_kinematics": False, "infer_gravity_response": False, "fit_new_force_or_action": False},
        "next_action": "Only if P2a passes, calibrate all four science frames independently with the frozen mapping and then audit exact history/provenance before any sky or pPXF operation."
    }
