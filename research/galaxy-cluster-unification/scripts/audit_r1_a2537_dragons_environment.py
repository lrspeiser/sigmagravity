#!/usr/bin/env python3
"""Run in the isolated DRAGONS environment and audit A2537 raw recognition."""
...
REPORT = ROOT / "results/r1_a2537_dragons_environment/report.json"
...
def main() -> None:
    ...
    protocol = json.loads((ROOT / "results/r1_a2537_gmos_reduction_protocol/report.json").read_text())
    if not protocol["authorization"]["audit_environment"]:
        raise RuntimeError("Pre-pixel A2537 control protocol did not authorize C1")
    ...
    bpm_record = next(record for record in provenance["records"] if record["archive_name"] == bpm_name)
    bpm_checksum = sha256(RAW / bpm_name) == bpm_record["local_sha256"] == config["raw_inputs"]["bad_pixel_mask"]["selected_sha256"]
    packages = {}
    ...
    gate = bool(version_gate and raw_gate)
    report = {
        "report_version": "R1B2-A2537-DRAGONS-control-environment-0.1",
