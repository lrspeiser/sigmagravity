#!/usr/bin/env python3
"""Audit that A2537 control-reduction choices predate science-array access."""
...
REPORT = ROOT / "results/r1_a2537_gmos_reduction_protocol/report.json"
...
def build_audit() -> dict:
    ...
    result_root = ROOT / "data/derived/r1_a2537_gmos_control"
    ...
    families = config["prerequisite_reports"]["preidentified_image_family_ids"]
    prerequisite_gate = bool(
        feasibility["gates"]["metadata_feasibility_gate_passed"]
        and raw["gates"]["raw_acquisition_gate_passed"]
        and raw["counts_as_non_disturbed_pilot"] is False
    )
    geometry_gate = bool(
        len(radii) == 3
        and max(radii) < max(edges)
        and len(set(families)) >= 2
        and config["profile_acceptance"]["minimum_independent_lens_families_inside_realized_support"] == 2
    )
    template_gate = template.exists() and sha256(template) == config["stellar_kinematic_fit"]["template_sha256"]
    pointing = config["spatial_extraction"]["pointing_geometry"]
    pointing_semantics_gate = bool(
        len(pointing["archive_pointing_along_slit_offsets_arcsec"]) == 4
        and max(pointing["archive_pointing_along_slit_offsets_arcsec"]) - min(pointing["archive_pointing_along_slit_offsets_arcsec"]) > 1.7
        and "Intentional along-slit pointing steps" in pointing["registration_rule"]
    )
    frozen_gate = bool(
        "before_any_science_array_inspection_or_reduction" in config["status"]
        and config["disturbed_control"] is True
        and config["counts_as_non_disturbed_pilot"] is False
    ...
    gate = prerequisite_gate and geometry_gate and template_gate and pointing_semantics_gate and frozen_gate
    report = {
        "report_version": "R1B2-A2537-GMOS-control-reduction-protocol-0.1",
        ...
        "pointing_step_semantics_frozen": pointing_semantics_gate,
        ...
        "gates": {
            ...
            "C1_environment_and_bpm_gate_passed": False,
