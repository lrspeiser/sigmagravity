#!/usr/bin/env python3
"""Run inside the isolated DRAGONS environment and audit raw-file recognition."""
...
import astrodata
import gemini_instruments  # noqa: F401 - import registers Gemini classes


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/r1_a1689_gmos_reduction_covariance_protocol.json"
RAW = ROOT / "data/raw/r1_a1689_gemini"
REPORT = ROOT / "results/r1_a1689_dragons_environment/report.json"
...
def main() -> None:
    ...
    biases = [
        inspect(name)
        for names in config["raw_inputs"]["night_bias_groups"].values()
        for name in names
    ]
    ...
    bpm_checksum = sha256(RAW / bpm_name) == bpm_provenance["selected"]["local_sha256"]
    ...
    recognition = {
        "science": all(expected_tags["science"].issubset(row["tags"]) for row in science),
        "flats": all(expected_tags["flat"].issubset(row["tags"]) for row in flats),
        "arcs": all(expected_tags["arc"].issubset(row["tags"]) for row in arcs),
        "biases": all(expected_tags["bias"].issubset(row["tags"]) for row in biases),
        "bpm": expected_tags["bpm"].issubset(bpm["tags"]),
    }
    version_gate = packages["dragons"] == "4.2.2" and packages["ppxf"] == "9.4.8"
    raw_gate = all(recognition.values()) and bpm_checksum
    report = {
        "report_version": "R1B1-A1689-DRAGONS-environment-0.1",
