#!/usr/bin/env bash
# Validate the frozen X4 SAS response interface on MOS2 output a01/input a02.
set -euo pipefail
...
interface_root=/home/henry/.local/share/sigmagravity-xmm/work/rxj2129/0093030201/x4/interface_v0_2_mos2_a01_from_a02
...
event_file=${background_root}/MOS2_corner_preserving_events.ds
output_spectrum=${x3_root}/a01_010_050kpc/MOS2/mos2S002-fovt.pi
input_spectrum=${x3_root}/a02_050_100kpc/MOS2/mos2S002-fovt.pi
detmap_counts=${interface_root}/mos2_uniform_detmap_counts.fits
detmap_zero=${interface_root}/mos2_uniform_detmap_zero.fits
detmap_uniform=${interface_root}/mos2_uniform_detmap.fits
output_rmf=${interface_root}/MOS2_output_a01.rmf
direct_arf=${interface_root}/MOS2_output_a01_direct.arf
cross_arf=${interface_root}/MOS2_output_a01_from_input_a02_cross.arf
central_arf=${interface_root}/MOS2_output_a01_from_central_source50.arf
...
reject_fatal_log() {
  ...
  if grep -Eiq '\*\* .*: error|detmapXBoundsExceeded|detmapYBoundsExceeded|zeroSumDetmap' "${log_file}"; then
    printf 'Fatal X4 interface record in %s\n' "${log_file}" >&2
    return 1
  fi
}

if [[ ! -f "${interface_root}/.detmap_complete" ]]; then
  evselect table="${event_file}:EVENTS" expression='(PI in [700:7000])&&(PATTERN<=12)&&(FLAG==0)' \
    ...
    writedss=yes updateexposure=yes \
    2>&1 | tee "${interface_root}/detmap_evselect.log"
...
if [[ ! -f "${interface_root}/.cross_arf_complete" ]]; then
  arfgen spectrumset="${output_spectrum}" crossreg_spectrumset="${input_spectrum}" \
    ...
    detmaptype=dataset detmaparray="${detmap_uniform}" filterdss=yes \
    extendedsource=yes modelee=yes psfmodel=ELLBETA \
    withbadpixcorr=yes badpixlocation="${event_file}" withbadpixres=yes \
    badpixelresolution=1 applyxcaladjustment=no applyabsfluxcorr=no \
...
if [[ ! -f "${interface_root}/.central_arf_complete" ]]; then
  arfgen spectrumset="${output_spectrum}" arfset="${central_arf}" \
    ...
    sourcey=0.08898852034328401 psfmodel=ELLBETA \
    withbadpixcorr=yes badpixlocation="${event_file}" withbadpixres=yes \
    badpixelresolution=1 applyxcaladjustment=no applyabsfluxcorr=no \
...
printf 'Frozen X4 MOS2 interface products: %s\n' "${interface_root}"
