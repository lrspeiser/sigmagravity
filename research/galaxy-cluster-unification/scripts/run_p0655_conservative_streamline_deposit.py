#!/usr/bin/env python3
"""Test conservative source-deposition transport on spent raw lensing."""
...
DEFAULT_CONFIG = ROOT / "configs" / "p0655_conservative_streamline_deposit.json"
...
def main():
    ...
    if protocol.get("status") != "frozen_before_any_P0655_score":
        raise RuntimeError("P0655 protocol is not frozen")
    ...
    p0653_report = read_json(ROOT / inputs["P0653_report"])
    ...
    if p0654_report["gate_results"]["CV_roots"]:
        raise RuntimeError("P0654 no longer has the topology failure under test")
    ...
    if not np.isclose(actual_padding, expected_padding):
        raise RuntimeError("P0655 padding is not exactly the inherited trace cap")
    ...
    cv_summary, fold_rows, prediction_frames = evaluate_candidate(
        ...
        stage="exact_conservative_deposit",
    )
    ...
    report = {
        "report_version": "P0655-CONSERVATIVE-STREAMLINE-DEPOSIT-RESULTS-1.0.0",
        ...
        "comparison": {
            ...
            "P0652_gather_CV_RMS_arcsec": float(
                p0652_report["comparison"]["primary_CV_RMS_arcsec"]
            ),
            "P0653_compact_CV_RMS_arcsec": float(
    ...
    labels = ["lambda 0", "matched m=3", "P0652 gather", "P0653 compact", "P0655 deposit"]
    values = [
        baseline_cv,
        multipole_cv,
        p0652_report["comparison"]["primary_CV_RMS_arcsec"],
    ...
    axes[0].set(ylabel="pooled exact CV RMS (arcsec)", title="Conservative transport")
    ...
    axes[1].set(
        ...
        title="Source-deposition fold stability",
    )
    figure.tight_layout()
    figure.savefig(output / "conservative_streamline_deposit.png", dpi=180)
    plt.close(figure)
    summary = f"""# P0655 conservative streamline deposit
