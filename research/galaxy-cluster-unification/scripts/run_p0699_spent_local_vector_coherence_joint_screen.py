#!/usr/bin/env python3
"""Run the frozen P0699 spent DDO154/RXJ2129 vector-coherence screen."""
...
DEFAULT_CONFIG = ROOT / "configs" / "p0699_spent_local_vector_coherence_joint_screen.json"
SPARC = ROOT / "data" / "raw" / "sparc"
MODEL = "local_vector_coherence_projected_routing_P0699"
...
def solve_joint_field(
    ...
    *,
    ...
    expected_source = (
        vector_coherence.coherence * coherent.equation_source
        + (1.0 - vector_coherence.coherence) * routing.local_generator_source
    )
    source_identity = relative_grid_rms(
        base.equation_source,
        expected_source,
        interior,
    )
    expected_joint = base.potential + fraction * (
        ...
        all(
            ...
            for array in (
                ...
                *coherent.acceleration,
                ...
                vector_coherence.coherence,
                vector_coherence.raw_coherence,
                vector_coherence.unsummed_acceleration_strength,
                *vector_coherence.direct_acceleration,
                base.potential,
                *base.acceleration,
                base.equation_source,
                joint.potential,
    ...
    audit = {
        ...
        "base_coherent_boundary_relative_mismatch": base_boundary_relative_mismatch(
            base,
            coherent,
        ),
        "routing_correction_boundary_relative_mismatch": routing_boundary,
        "coherence_minimum": float(np.min(vector_coherence.coherence)),
        "coherence_median": float(np.median(vector_coherence.coherence)),
        "coherence_maximum": float(np.max(vector_coherence.coherence)),
        "raw_triangle_inequality_excess_maximum": (
            vector_coherence.maximum_triangle_inequality_excess
        ),
...
def main():
    ...
    if protocol.get("status") != (
        "frozen_before_any_P0699_DDO154_rotation_or_RXJ2129_field_fit_or_topology_score"
    ):
        raise RuntimeError("P0699 protocol is not frozen")
    equation = protocol["equation"]
    expected = protocol["predeclared_integrity_gates"]
    parent_paths = {
        "P0698": ROOT / protocol["math_parent"],
    ...
    integrity = {
        "P0698_status": parents["P0698"].get("status") == expected["P0698_status"],
        "P0698_math_pass": bool(parents["P0698"].get("all_math_gates_pass"))
        is bool(expected["P0698_all_math_gates_pass"]),
        "P0698_advanced": bool(
            parents["P0698"].get("candidate_advanced_to_spent_joint_screen")
        )
        is bool(expected["P0698_candidate_advanced_to_spent_joint_screen"]),
        ...
        "no_fitted_coherence": int(equation["fitted_coherence_parameters"])
        == int(expected["fitted_coherence_parameters"]),
    ...
    if not all(integrity.values()):
        raise RuntimeError(f"P0699 integrity failure before scores: {integrity}")

    output = ROOT / protocol["outputs"]["directory"]
    output.mkdir(parents=True, exist_ok=True)

    print("P0699: solving spent DDO154 vector-coherence joint field", flush=True)
    ...
    galaxy_curve.insert(0, "variant", "local_vector_coherence_projected_routing")
    sparc_curve = next(
        curve for curve in load_curves(SPARC) if curve.metadata.name == "DDO154"
    )
    galaxy_x, galaxy_y = np.meshgrid(galaxy_axis, galaxy_axis, indexing="ij")
    galaxy_score_annulus = (
        (np.hypot(galaxy_x, galaxy_y) >= float(np.min(sparc_curve.radius_kpc)))
        & (np.hypot(galaxy_x, galaxy_y) <= float(np.max(sparc_curve.radius_kpc)))
    )
    galaxy_midplane = len(galaxy_axis) // 2
    galaxy_score_coherence = galaxy_vector_coherence.coherence[
        :, :, galaxy_midplane
    ][galaxy_score_annulus]
    ...
    np.savez_compressed(
        ...
        coherence=galaxy_vector_coherence.coherence,
        base_potential_km2_s2=galaxy_base.potential,
    ...
    print("P0699: solving spent RX J2129 vector-coherence joint field", flush=True)
    ...
    if not np.isclose(map_a0, float(equation["a0_si_m_s2"]), rtol=0.0, atol=0.0):
        raise RuntimeError("P0699 cluster a0 no longer matches the registered map")
    cluster_spacing_m = float(cluster_axis[1] - cluster_axis[0]) * KPC_M
    (
        cluster_joint,
        cluster_base,
        cluster_coherent,
        cluster_vector_coherence,
        cluster_routing,
        cluster_local,
        cluster_fraction,
        cluster_covariance,
    ...
    cluster_annulus_3d = np.broadcast_to(
        annulus[:, :, None],
        cluster_vector_coherence.coherence.shape,
    )
    cluster_lens_coherence = cluster_vector_coherence.coherence[cluster_annulus_3d]
    coherence_summary = pd.DataFrame(
        [
            {
                "system": "DDO154",
                "region": "SPARC_score_radius_midplane",
                "cells": int(galaxy_score_coherence.size),
                "coherence_minimum": float(np.min(galaxy_score_coherence)),
                "coherence_median": float(np.median(galaxy_score_coherence)),
                "coherence_mean": float(np.mean(galaxy_score_coherence)),
                "coherence_maximum": float(np.max(galaxy_score_coherence)),
            },
            {
                "system": "RXJ2129",
                "region": "strong_lens_projected_annulus_all_z",
                "cells": int(cluster_lens_coherence.size),
                "coherence_minimum": float(np.min(cluster_lens_coherence)),
                "coherence_median": float(np.median(cluster_lens_coherence)),
                "coherence_mean": float(np.mean(cluster_lens_coherence)),
                "coherence_maximum": float(np.max(cluster_lens_coherence)),
            },
        ]
    )
    coherence_summary.to_csv(
        output / protocol["outputs"]["coherence_summary"],
        index=False,
    )
    cluster_field = {
        **cluster_audit,
    ...
    np.savez_compressed(
        ...
        coherence=cluster_vector_coherence.coherence,
        base_potential_m2_s2=cluster_base.potential,
    ...
    if not integrity["raw_coverage"]:
        raise RuntimeError("P0699 raw-image coverage changed")
    ...
    roots.to_csv(output / "rxj2129_vector_coherence_global_roots.csv", index=False)
    assignments.to_csv(
        output / "rxj2129_vector_coherence_global_assignments.csv",
        index=False,
    )
    ...
    maximum_component_residual = max(
        ...
        for key in (
            ...
            "gated_component_normalized_residual_RMS",
        )
    )
    maximum_source_identity = max(
        float(audit["gated_source_identity_relative_RMS"]) for audit in audits
    )
    maximum_joint_identity = max(
        float(audit["hybrid_potential_identity_relative_RMS"]) for audit in audits
    )
    maximum_base_boundary = max(
        float(audit["base_coherent_boundary_relative_mismatch"]) for audit in audits
    )
    maximum_routing_boundary = max(
    ...
    coherence_minimum = min(float(audit["coherence_minimum"]) for audit in audits)
    coherence_maximum = max(float(audit["coherence_maximum"]) for audit in audits)
    triangle_excess = max(
        float(audit["raw_triangle_inequality_excess_maximum"]) for audit in audits
    )
    gates = protocol["predeclared_advancement_gates"]
    gate_results = {
        **integrity,
        "coherence_lower": coherence_minimum >= float(gates["coherence_min"]),
        "coherence_upper": coherence_maximum <= float(gates["coherence_max"]),
        "triangle_inequality": triangle_excess
        ...
        is bool(
            gates[
                "all_potentials_accelerations_sources_coherences_and_deflections_finite"
            ]
        ),
        "component_residuals": maximum_component_residual
        <= float(
            gates[
                "newtonian_routing_and_gated_component_normalized_residual_RMS_max"
            ]
        ),
        ...
        "accounting_no_fitted_coherence": int(equation["fitted_coherence_parameters"])
        == int(gates["fitted_coherence_parameters"]),
    ...
    axes[0, 1].bar(
        ["DDO154", "RX J2129"],
        [
            float(np.median(galaxy_score_coherence)),
            float(np.median(cluster_lens_coherence)),
        ],
        color=["C1", "C3"],
    )
    axes[0, 1].set(
        title="Local vector coherence in score regions",
    ...
    figure.suptitle("P0699 spent local-vector-coherence joint screen")
    figure.tight_layout()
    figure.savefig(output / protocol["outputs"]["figure"], dpi=180)
    plt.close(figure)

    report = {
        "report_version": "P0699-SPENT-LOCAL-VECTOR-COHERENCE-JOINT-SCREEN-RESULTS-1.0.0",
        ...
        "spent_DDO154": {
            ...
            "score_region_coherence": coherence_summary.iloc[0].to_dict(),
        ...
        "spent_RXJ2129": {
            ...
            "field": cluster_field,
            "strong_lens_region_coherence": coherence_summary.iloc[1].to_dict(),
            "fit": cluster_fit,
        ...
        "coherence_range": [coherence_minimum, coherence_maximum],
        "raw_triangle_inequality_excess_maximum": triangle_excess,
    ...
    summary = f"""# P0699 spent local-vector-coherence joint screen
...
- DDO154 / RX J2129 median score-region coherence: **{np.median(galaxy_score_coherence):.4g} / {np.median(cluster_lens_coherence):.4g}**.
