#!/usr/bin/env python3
"""Run the frozen P0701 spent DDO154/RXJ2129 radial-alignment screen."""
...
from voidscreen.barycentric_radial_alignment import barycentric_radial_alignment
...
DEFAULT_CONFIG = ROOT / "configs" / "p0701_spent_barycentric_radial_alignment_joint_screen.json"
SPARC = ROOT / "data" / "raw" / "sparc"
MODEL = "barycentric_radial_alignment_projected_routing_P0701"
...
def solve_joint_field(
    ...
    *,
    ...
    radial_alignment = barycentric_radial_alignment(
        density,
        routing.newtonian.acceleration,
        spacing,
    )
    ...
    expected_source = (
        radial_alignment.alignment * coherent.equation_source
        + (1.0 - radial_alignment.alignment) * routing.local_generator_source
    )
        ...
        all(
            ...
            for array in (
                ...
                *coherent.acceleration,
                ...
                radial_alignment.alignment,
                radial_alignment.inward_radial_acceleration,
                radial_alignment.acceleration_magnitude,
                base.potential,
                *base.acceleration,
                base.equation_source,
                joint.potential,
    ...
    audit = {
        ...
        "alignment_minimum": float(np.min(radial_alignment.alignment)),
        "alignment_median": float(np.median(radial_alignment.alignment)),
        "alignment_maximum": float(np.max(radial_alignment.alignment)),
...
def main():
    ...
    if protocol.get("status") != (
        "frozen_before_any_P0701_DDO154_rotation_or_RXJ2129_field_fit_or_topology_score"
    ):
        raise RuntimeError("P0701 protocol is not frozen")
    equation = protocol["equation"]
    expected = protocol["predeclared_integrity_gates"]
    parent_paths = {
        "P0700": ROOT / protocol["math_parent"],
    ...
    integrity = {
        "P0700_status": parents["P0700"].get("status") == expected["P0700_status"],
        "P0700_math_pass": bool(parents["P0700"].get("all_math_gates_pass"))
        is bool(expected["P0700_all_math_gates_pass"]),
        "P0700_advanced": bool(
            parents["P0700"].get("candidate_advanced_to_spent_joint_screen")
        )
        is bool(expected["P0700_candidate_advanced_to_spent_joint_screen"]),
        ...
        "no_fitted_alignment": int(equation["fitted_alignment_parameters"])
        == int(expected["fitted_alignment_parameters"]),
    ...
    if not all(integrity.values()):
        raise RuntimeError(f"P0701 integrity failure before scores: {integrity}")

    output = ROOT / protocol["outputs"]["directory"]
    output.mkdir(parents=True, exist_ok=True)

    print("P0701: solving spent DDO154 barycentric-alignment joint field", flush=True)
    ...
    galaxy_curve.insert(0, "variant", "barycentric_radial_alignment_projected_routing")
    sparc_curve = next(
    ...
    galaxy_score_alignment = galaxy_radial_alignment.alignment[
        :, :, galaxy_midplane
    ][galaxy_score_annulus]
    ...
    np.savez_compressed(
        ...
        alignment=galaxy_radial_alignment.alignment,
    ...
    print("P0701: solving spent RX J2129 barycentric-alignment joint field", flush=True)
    ...
    if not np.isclose(map_a0, float(equation["a0_si_m_s2"]), rtol=0.0, atol=0.0):
        raise RuntimeError("P0701 cluster a0 no longer matches the registered map")
    cluster_spacing_m = float(cluster_axis[1] - cluster_axis[0]) * KPC_M
    (
        cluster_joint,
        cluster_base,
        cluster_coherent,
        cluster_radial_alignment,
        cluster_routing,
        cluster_local,
        cluster_fraction,
        cluster_covariance,
    ...
    cluster_annulus_3d = np.broadcast_to(
        annulus[:, :, None],
        cluster_radial_alignment.alignment.shape,
    )
    cluster_lens_alignment = cluster_radial_alignment.alignment[cluster_annulus_3d]
    alignment_summary = pd.DataFrame(
        [
            {
                "system": "DDO154",
                "region": "SPARC_score_radius_midplane",
                "cells": int(galaxy_score_alignment.size),
                "alignment_minimum": float(np.min(galaxy_score_alignment)),
                "alignment_median": float(np.median(galaxy_score_alignment)),
                "alignment_mean": float(np.mean(galaxy_score_alignment)),
                "alignment_maximum": float(np.max(galaxy_score_alignment)),
            },
            {
                "system": "RXJ2129",
                "region": "strong_lens_projected_annulus_all_z",
                "cells": int(cluster_lens_alignment.size),
                "alignment_minimum": float(np.min(cluster_lens_alignment)),
                "alignment_median": float(np.median(cluster_lens_alignment)),
                "alignment_mean": float(np.mean(cluster_lens_alignment)),
                "alignment_maximum": float(np.max(cluster_lens_alignment)),
            },
        ]
    )
    alignment_summary.to_csv(
        output / protocol["outputs"]["alignment_summary"],
        index=False,
    )
    cluster_field = {
        **cluster_audit,
    ...
    np.savez_compressed(
        ...
        alignment=cluster_radial_alignment.alignment,
    ...
    if not integrity["raw_coverage"]:
        raise RuntimeError("P0701 raw-image coverage changed")
    ...
    roots.to_csv(output / "rxj2129_radial_alignment_global_roots.csv", index=False)
    assignments.to_csv(
        output / "rxj2129_radial_alignment_global_assignments.csv",
        index=False,
    )
    ...
    alignment_minimum = min(float(audit["alignment_minimum"]) for audit in audits)
    alignment_maximum = max(float(audit["alignment_maximum"]) for audit in audits)
    gates = protocol["predeclared_advancement_gates"]
    gate_results = {
        **integrity,
        "alignment_lower": alignment_minimum >= float(gates["alignment_min"]),
        "alignment_upper": alignment_maximum <= float(gates["alignment_max"]),
        ...
        is bool(
            gates[
                "all_potentials_accelerations_sources_alignments_and_deflections_finite"
            ]
        ),
        ...
        "accounting_no_fitted_alignment": int(equation["fitted_alignment_parameters"])
        == int(gates["fitted_alignment_parameters"]),
    ...
    axes[0, 0].plot(
        ...
        label="barycentric-alignment joint",
    )
    ...
    axes[0, 1].bar(
        ["DDO154", "RX J2129"],
        [
            float(np.median(galaxy_score_alignment)),
            float(np.median(cluster_lens_alignment)),
        ],
        color=["C1", "C3"],
    )
    axes[0, 1].set(
        title="Local radial alignment in score regions",
        ylabel="median alignment",
        ylim=(0, 1),
    )
    axes[1, 0].bar(
        ["training", "heldout"],
    ...
    figure.suptitle("P0701 spent barycentric radial-alignment joint screen")
    figure.tight_layout()
    figure.savefig(output / protocol["outputs"]["figure"], dpi=180)
    plt.close(figure)

    report = {
        "report_version": "P0701-SPENT-BARYCENTRIC-RADIAL-ALIGNMENT-JOINT-SCREEN-RESULTS-1.0.0",
        ...
        "operator_source_sha256": sha256(ROOT / "src/voidscreen/barycentric_radial_alignment.py"),
        ...
        "spent_DDO154": {
            ...
            "score_region_alignment": alignment_summary.iloc[0].to_dict(),
        ...
        "spent_RXJ2129": {
            ...
            "field": cluster_field,
            "strong_lens_region_alignment": alignment_summary.iloc[1].to_dict(),
            "fit": cluster_fit,
        ...
        "alignment_range": [alignment_minimum, alignment_maximum],
    ...
    summary = f"""# P0701 spent barycentric radial-alignment joint screen
...
- DDO154 / RX J2129 median score-region alignment: **{np.median(galaxy_score_alignment):.4g} / {np.median(cluster_lens_alignment):.4g}**.
