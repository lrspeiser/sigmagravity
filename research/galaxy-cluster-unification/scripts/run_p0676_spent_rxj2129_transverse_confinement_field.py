#!/usr/bin/env python3
"""Solve the frozen spent RX J2129 transverse-confinement field."""
...
def main():
    ...
    if protocol.get("status") != "frozen_before_any_P0676_field_or_deflection_score":
        raise RuntimeError("P0676 protocol is not frozen")
    ...
    print("solving transverse-confinement AQUAL", flush=True)
    confinement = solve_transverse_confinement_aqual_3d(
        source,
        spacing_m,
        boundary,
        sigma,
    ...
    print(
        f"confinement: converged={confinement.converged} "
        f"residual={confinement.normalized_residual_rms:.6g} "
        f"iterations={confinement.nonlinear_iterations}",
        flush=True,
    )
    deflection = photon_deflection_zero_slip(
        confinement.acceleration,
        spacing_m,
        distance_ratio=1.0,
    )
    ...
    confinement_magnitude = np.hypot(
        deflection.alpha_x_arcsec,
        deflection.alpha_y_arcsec,
    )
    scalar_rms = vector_rms(scalar_alpha_x, scalar_alpha_y, annulus)
    confinement_rms = vector_rms(
        deflection.alpha_x_arcsec,
        deflection.alpha_y_arcsec,
        annulus,
    )
    response_ratio = confinement_rms / max(scalar_rms, np.finfo(float).tiny)
    ...
    confinement_curl = normalized_deflection_curl(
        deflection.alpha_x_arcsec,
        deflection.alpha_y_arcsec,
    ...
    boundary_mismatch = float(
        np.max(np.abs(confinement.potential[boundary_cells] - boundary[boundary_cells]))
    ) / boundary_scale
    finite = bool(
        np.all(np.isfinite(confinement.potential))
        and all(np.all(np.isfinite(component)) for component in confinement.acceleration)
        and np.all(np.isfinite(confinement_magnitude))
    )
    minimum_eigenvalue = float(
        confinement.metadata["minimum_constitutive_eigenvalue"]
    )
    scalar_median = float(np.median(scalar_magnitude[annulus]))
    confinement_median = float(np.median(confinement_magnitude[annulus]))

    failed_topology = failure_parent["topology"]["compound_absolute_P0673"]
    gates = protocol["predeclared_progression_gates"]
    equations = protocol["field_equations"]
    gate_results = {
        "P0675_failed": failure_parent["status"] == gates["P0675_status"],
        "P0675_missing_multiplicity": int(
            failed_topology["missing_multiplicity_families"]
        )
        == int(gates["P0675_missing_multiplicity_families"]),
        "P0675_no_critical_curves": int(
            failed_topology["critical_curve_present_families"]
        )
        == int(gates["P0675_critical_curve_present_families"]),
        ...
        "confinement_residual": confinement.normalized_residual_rms
        <= float(gates["confinement_normalized_residual_RMS_max"]),
        "confinement_convergence": confinement.converged
        is bool(gates["confinement_solver_converged"]),
        ...
        is bool(gates["minimum_confinement_constitutive_eigenvalue_strictly_positive"]),
        "scalar_reference_present": scalar_median
        >= float(
            gates[
                "scalar_reference_strong_lens_median_physical_deflection_arcsec_min"
            ]
        ),
        "confinement_deflection_present": confinement_median
        >= float(gates["confinement_strong_lens_median_physical_deflection_arcsec_min"]),
        "response_ratio_lower": response_ratio
        >= float(gates["confinement_to_scalar_strong_lens_deflection_RMS_ratio_min"]),
        "response_ratio_upper": response_ratio
        <= float(gates["confinement_to_scalar_strong_lens_deflection_RMS_ratio_max"]),
        "confinement_change_nonperturbative": relative_difference
        >= float(gates["confinement_minus_scalar_strong_lens_relative_RMS_min"]),
        "confinement_change_stable": relative_difference
        <= float(gates["confinement_minus_scalar_strong_lens_relative_RMS_max"]),
        "confinement_curl": confinement_curl
        <= float(gates["confinement_normalized_deflection_curl_RMS_max"]),
    ...
    metrics = {
        ...
        "confinement_normalized_residual_RMS": confinement.normalized_residual_rms,
        "confinement_nonlinear_iterations": confinement.nonlinear_iterations,
        "boundary_maximum_relative_mismatch": boundary_mismatch,
        "minimum_confinement_constitutive_eigenvalue": minimum_eigenvalue,
        "scalar_strong_lens_median_physical_deflection_arcsec": scalar_median,
        "confinement_strong_lens_median_physical_deflection_arcsec": confinement_median,
        "scalar_strong_lens_deflection_RMS_arcsec": scalar_rms,
        "confinement_strong_lens_deflection_RMS_arcsec": confinement_rms,
        "confinement_to_scalar_strong_lens_deflection_RMS_ratio": response_ratio,
        "confinement_minus_scalar_strong_lens_relative_RMS": relative_difference,
        "confinement_normalized_deflection_curl_RMS": confinement_curl,
    ...
    np.savez_compressed(
        ...
        confinement_potential_m2_s2=confinement.potential,
        scalar_alpha_x_physical_arcsec=scalar_alpha_x,
        scalar_alpha_y_physical_arcsec=scalar_alpha_y,
        confinement_alpha_x_physical_arcsec=deflection.alpha_x_arcsec,
        confinement_alpha_y_physical_arcsec=deflection.alpha_y_arcsec,
        confinement_sigma=sigma,
        transport_direction_x=directions[0],
        transport_direction_y=directions[1],
        transport_direction_z=directions[2],
    )
    report = {
        "report_version": "P0676-SPENT-RXJ2129-TRANSVERSE-CONFINEMENT-FIELD-RESULTS-1.0.0",
    ...
    titles = (
        ...
        "transverse-confinement deflection",
        "confinement - scalar",
    )
    ...
    figure.tight_layout()
    figure.savefig(output / "p0676_transverse_confinement_fields.png", dpi=180)
    plt.close(figure)

    failed = [name for name, passed in gate_results.items() if not passed]
    summary = f"""# P0676 spent RX J2129 transverse-confinement field

- Status: **{'PASS' if all_pass else 'FAIL'}**.
- Confinement residual / iterations: **{confinement.normalized_residual_rms:.4g} / {confinement.nonlinear_iterations}**.
- Scalar/confinement strong-lens median physical deflection: **{scalar_median:.4g} / {confinement_median:.4g} arcsec**.
- Confinement/scalar deflection RMS ratio: **{response_ratio:.6g}**.
- Confinement-minus-scalar relative RMS: **{relative_difference:.6g}**.
- Confinement normalized curl: **{confinement_curl:.3g}**.
