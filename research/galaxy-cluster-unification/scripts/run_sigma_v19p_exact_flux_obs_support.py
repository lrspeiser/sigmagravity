#!/usr/bin/env python3
"""Validate exact per-observation flux_obs support and enumerate V19P tasks."""
...
DEFAULT_CONFIG = ROOT / "configs" / "sigma_v19p_exact_flux_obs_support.json"
DEFAULT_OUTPUT = ROOT / "results" / "sigma_v19p_exact_flux_obs_support"
...
def validate_parent_hashes(config: dict[str, Any]) -> None:
    for key, value in config["parents"].items():
        ...
        if expected is not None and sha256(ROOT / value) != expected:
            raise RuntimeError(f"V19P parent hash mismatch: {value}")
...
def frozen_product(row: dict[str, Any], role: str) -> Path:
    matches = [item for item in row["frozen_snapshot"]["products"] if item["role"] == role]
    if len(matches) != 1:
        raise RuntimeError(f"expected one {role} product for {row['cluster']}")
    item = matches[0]
    path = ROOT / item["relative_path"]
    if path.stat().st_size != item["bytes"] or sha256(path) != item["sha256"]:
        raise RuntimeError(f"frozen source-map product changed: {path}")
    return path
...
def inventory_rows(inventory: dict[str, Any]) -> dict[tuple[str, int, str], dict[str, Any]]:
    result = {}
    for cluster in inventory["clusters"]:
        ...
        for observation in cluster["observations"]:
            ...
            for product in observation["products"]:
                key = (name, obsid, product["role"])
                if key in result:
                    raise RuntimeError(f"duplicate inventory product: {key}")
                path = Path(product["path"])
                if path.stat().st_size != int(product["bytes"]):
                    raise RuntimeError(f"V19P input size changed: {path}")
                if sha256(path) != product["sha256"]:
                    raise RuntimeError(f"V19P input hash changed: {path}")
                result[key] = product
    return result
...
def image_region_counts(counts: np.ndarray, binmap: np.ndarray) -> dict[int, int]:
    values = np.nan_to_num(counts, nan=0.0, posinf=0.0, neginf=0.0)
    rounded = np.rint(values)
    if not np.array_equal(values, rounded) or np.any(rounded < 0):
        raise RuntimeError("flux_obs count image is not a finite nonnegative integer image")
    labels = binmap[binmap >= 0].astype(int)
    weights = rounded[binmap >= 0].astype(np.int64)
    totals = np.bincount(labels, weights=weights, minlength=int(labels.max()) + 1)
    return {index: int(value) for index, value in enumerate(totals) if value > 0}


def parent_task_keys(path: Path) -> set[tuple[str, int, int, int]]:
...
def build_cluster(
    ...
    binmap = image(region_product(regions, "binmap")).astype(int)
    combined_counts = image(frozen_product(source, "broad_counts"))
    combined_background = image(frozen_product(source, "broad_scaled_background"))
    if combined_counts.shape != binmap.shape or combined_background.shape != binmap.shape:
        raise RuntimeError(f"V19P image-shape mismatch for {cluster}")
    valid_ids = {int(value) for value in regions["valid_region_ids"]}
    all_labels = binmap >= 0
    energy = tuple(int(value) for value in config["exact_support"]["energy_eV"])
    scale_by_obs_ccd = {
    ...
    for observation in sorted(source["observations"], key=lambda row: int(row["obsid"])):
        ...
        obs_counts = image(Path(obs_counts_record["path"]))
        if obs_counts.shape != binmap.shape:
            raise RuntimeError(f"V19P observation image shape mismatch: {cluster} {obsid}")
        summed_observation_images += np.nan_to_num(obs_counts, nan=0.0)
        exact_by_region = image_region_counts(obs_counts, binmap)
        fov_path = Path(fov_record["path"])
        science, assigned, science_ccds = event_assignments(
            f"{science_record['path']}[sky=region({fov_path})]",
            binmap,
            source["grid"],
            energy,
        )
        background, background_assigned, background_ccds = event_assignments(
            str(background_record["path"]),
            binmap,
            source["grid"],
            energy,
        )
        ...
        mismatched_regions = {key: value for key, value in region_deltas.items() if value}
        task_count = 0
        ...
        observation_rows.append(
            {
                ...
                "exact_fov_filtered_events_inside_all_bins": assigned,
                "science_count_delta": assigned - exact_total,
                "region_count_with_nonzero_delta": len(mismatched_regions),
                "maximum_absolute_region_delta": max(
                    (abs(value) for value in mismatched_regions.values()), default=0
                ),
    ...
    return (
        {
            ...
            "exact_fov_filtered_science_events_inside_all_bins": science_total,
...
def main() -> None:
    ...
    inventory_report = load_json(ROOT / config["parents"]["input_inventory"])
    if inventory_report["integrity"]["fits_array_opened"]:
        raise RuntimeError("V19P inventory was not hash-only")
    inventory = inventory_rows(inventory_report)
    expected_keys = parent_task_keys(ROOT / config["parents"]["v19n_manifest"])
    ...
    total_expected = int(config["sample"]["expected_response_task_count_total"])
    cluster_gates_pass = all(all(row["gates"].values()) for row in cluster_rows)
    total_task_gate = len(tasks) == total_expected
    passed = cluster_gates_pass and total_task_gate
    report = {
        "status": (
            "exact_flux_obs_support_conserved_and_response_workload_authorized"
            if passed
            else "exact_flux_obs_support_or_response_workload_gate_failed"
        ),
        "protocol_version": config["protocol_version"],
        "generated_utc": datetime.now(UTC).isoformat(),
        "config_sha256": sha256(config_path),
        "runner_sha256": sha256(Path(__file__).resolve()),
        "input_inventory_sha256": sha256(ROOT / config["parents"]["input_inventory"]),
        "manifest": {
        ...
        "global_gates": {
            "all_cluster_gates_pass": cluster_gates_pass,
            "total_task_count_equals_5082": total_task_gate,
        },
