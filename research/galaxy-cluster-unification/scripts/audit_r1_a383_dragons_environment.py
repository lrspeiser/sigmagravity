#!/usr/bin/env python3
"""Run in the isolated DRAGONS environment and audit A383 raw recognition."""
...
REPORT = ROOT / "results/r1_a383_dragons_environment/report.json"
...
def main() -> None:
    ...
    flat_names = sorted({pair[0] for pair in mapping.values()} | set(config["raw_inputs"]["flat_sensitivity_controls"]))
    science = [inspect(name) for name in mapping]
    flats = [inspect(name) for name in flat_names]
    ...
    report = {
        "report_version": "R1B2-A383-DRAGONS-environment-0.1",
