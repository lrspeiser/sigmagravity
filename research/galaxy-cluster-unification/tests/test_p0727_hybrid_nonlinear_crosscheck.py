...
def test_frozen_hybrids_preserve_physics_and_known_answer() -> None:
    config = json.loads(
        (ROOT / "configs" / "p0727_hybrid_nonlinear_crosscheck.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    for variant in config["solverVariants"]:
        ...
        assert model["solver"]["picardWarmupIterations"] < model["solver"]["maxIterations"]
