...
SCRIPT = ROOT / "scripts" / "run_sigma_v19aj_fors1_core_astrometry.py"
CONFIG = ROOT / "configs" / "sigma_v19aj_fors1_core_astrometry.json"
SPEC = importlib.util.spec_from_file_location("sigma_v19aj", SCRIPT)
...
def test_compact_core_centroid_recovers_synthetic_gaussian():
...
def test_only_centroid_geometry_changed_from_v19ai():
    ...
    assert current["parent_artifacts"] == previous["parent_artifacts"]
    assert current["science_products"] == previous["science_products"]
    assert current["gates"] == previous["gates"]
    assert current["centroid"]["maximum_shift_from_v19ah_peak_pixel"] == 2.0
    for name in ("minimum_fwhm_pixel", "maximum_fwhm_pixel", "maximum_ellipticity"):
