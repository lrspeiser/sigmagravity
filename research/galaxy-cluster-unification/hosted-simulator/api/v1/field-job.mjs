...
function route(request) {
  ...
  if (resource === undefined && name === undefined) return { path: `api/v1/field-jobs/${id}`, method: "GET" };
  if (resource === "events" && name === undefined) return { path: `api/v1/field-jobs/${id}/events`, method: "GET" };
  if (resource === "artifacts" && name === undefined) return { path: `api/v1/field-jobs/${id}/artifacts`, method: "GET" };
  if (resource === "cancel" && name === undefined) return { path: `api/v1/field-jobs/${id}/cancel`, method: "POST" };
  if (
    resource === "artifact"
    && typeof name === "string"
    && name.length > 0
    ...
    return { path: `api/v1/field-jobs/${id}/artifacts/${encodeURIComponent(name)}`, method: "GET" };
  }
  return null;
}
