...
export function createLocalFieldJobRouter(service) {
  return async function route(request, response, url) {
    if (!url.pathname.startsWith("/api/v1/data-uploads") && !url.pathname.startsWith("/api/v1/field-jobs") && !url.pathname.startsWith("/api/v1/galaxy-jobs") && !url.pathname.startsWith("/api/v1/observation-evaluation-jobs") && !url.pathname.startsWith("/api/v1/inverse-response-jobs")) return false;
    ...
    try {
      ...
      if (url.pathname === "/api/v1/inverse-response-jobs") {
        if (request.method === "POST") send(response, 202, await service.createInverseResponseJob(request.body));
        else if (request.method === "GET") send(response, 200, await service.listInverseResponseJobs());
        else if (request.method === "OPTIONS") { setCors(response); response.status(204).end(); }
        else method(response, ["GET", "POST"]);
        return true;
      }
      const inverseJobMatch = url.pathname.match(/^\/api\/v1\/inverse-response-jobs\/(job_[0-9a-f]{24})(?:\/(events|artifacts|cancel)(?:\/(.+))?)?$/);
      if (inverseJobMatch) {
        const [, id, resource, artifactName] = inverseJobMatch;
        await service.getInverseResponseJob(id);
        if (!resource && request.method === "GET") send(response, 200, await service.getInverseResponseJob(id));
        ...
        } else if (resource === "cancel" && request.method === "POST") send(response, 200, await service.cancelInverseResponseJob(id));
        else if (request.method === "OPTIONS") { setCors(response); response.status(204).end(); }
