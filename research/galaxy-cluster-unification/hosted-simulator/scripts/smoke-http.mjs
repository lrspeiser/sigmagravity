...
assert.match(guide, /Generate many plausible baryonic galaxies, not one invented 3D truth/);
...
assert.match(guide, /Score a cylindrical field against galaxy motion/);
assert.match(guide, /axisymmetric_midplane_direct/);
assert.match(guide, /Project a cylindrical field into a lensing map/);
assert.match(guide, /axisymmetric_cylindrical_ray_integral/);
assert.match(guide, /Score raw image positions from a cylindrical field/);
assert.match(guide, /zero_only_outside_verified_root_support/);
assert.match(guide, /Neon's Marketplace terms/);

const health = await request("/api/v1/health");
assert.equal(health.status, "ok");
assert.equal(health.version, "0.35.0-preview");
...
assert.ok(["not_configured", "configured", "verified_consumed"].includes(storageReadiness.queue.state));
assert.ok(["not_configured", "misconfigured", "verified_migrated", "migration_required", "verification_failed"].includes(storageReadiness.jobMetadataDatabase.state));
assert.ok(["not_configured", "credential_configured_trigger_verification_required", "verified"].includes(storageReadiness.outboxScheduler.state));
assert.ok(["not_configured", "configured"].includes(storageReadiness.statelessScientificContainer.state));
