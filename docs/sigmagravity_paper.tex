...
The multiplicative operator structure $g_{\rm eff}(x) = g_{\rm bar}(x)[1+K(x)]$ is motivated by stationary-phase reduction of gravitational path integrals. The coherence window $C(R)$ uses a Burr-XII form justified by superstatistical decoherence models (Appendix C). Axisymmetry guarantees curl-free fields; ring integrals reduce to elliptic integrals (Appendix B); Solar System safety follows from $K\to0$ as $R\to0$.
...
\textbf{Effective field (domain‑agnostic):}
...
\textbf{Coherence window:}

\begin{equation}
C(R) = 1 - [1 + (R/\ell_0)^p]^{-n_{\rm coh}}
\end{equation}

with $\allowbreak \ell_0 = c\,\tau_{\rm collapse}$\allowbreak .


\textbf{Canonical kernel:}
...
\textbf{Exact ring geometry:} the azimuthal Green's function reduces to complete elliptic integrals with parameter
...
• \textbf{Gas}: gNFW pressure profile (Arnaud+2010), normalized to $f_{\rm gas}(R_{500})=0.11$ with clumping correction $C(r)$.
...
Sampling via PyMC \textbf{NUTS} on a differentiable $\theta_E$ grid surrogate (target\_accept$=0.95$); WAIC/LOO used for model comparison ($\Delta$WAIC $\approx 0 \pm 2.5$).
...
\item Projected kernel: core/kernel2d\_sigma.py applies $K_\Sigma(R)=A_c \cdot C(R)$ with $C(R)=1-[1+(R/\ell_0)^p]^{-n_{\rm coh}}$.
\item Diagnostics: point/mean convergence, cumulative mass \& boost, 2‑D maps, Einstein‑mass check.
\end{enumerate}

Proof‑of‑concept (MACS0416): with spherical geometry, the calibrated model gives $\theta_E = 30.4''$ (obs $30.0''$), $\langle\kappa\rangle(<R_E)=1.019$. Triaxial tests retain $\sim21.5\%$ $\theta_E$ variation across plausible axis ratios, as expected.
...
• Population: $A_{c,i} \sim N(\mu_A,\sigma_A)$, optionally adding geometry ($q_{\rm plane}$, $q_{\rm LOS}$) and small $\kappa_{\rm ext}$.  

• Likelihood: $\chi^2 = \sum_i (\theta_{E,i}^{\rm model}-\theta_{E,i}^{\rm obs})^2/\sigma_i^2$, with Tier‑1 (relaxed) priority.
...
\textbf{MACS0416:} $\theta_E^{\rm pred} = 30.43''$ vs $30.0''$ observed (1.4\% error). Geometry sensitivity preserved ($\sim21.5\%$ spread across tested $q_p$, $q_{\rm los}$). Boost at $R_E$ $\sim 7\times$ relative to Newtonian $\kappa$.


\subsection{Clusters (hierarchical NUTS‑grid; N≈10 + blind hold‑outs)}


Using a hierarchical calibration on a curated tier‑1/2 sample ($N\approx10$), together with triaxial projection, source‑redshift distributions $P(z_s)$, and baryonic surface‑density profiles $\Sigma_{\rm baryon}(R)$ (gas + BCG/ICL), the $\Sigma$-Gravity kernel reproduces Einstein radii without invoking particle dark-matter halos in these calculations. In a blind hold‑out test on Abell 2261 and MACS J1149.5+2223, posterior‑suggestive coverage is 2/2 inside the 68\% interval (coverage = fraction of observed $\theta_E$ inside the model's 68\% posterior‑suggestive interval, PPC) and the median fractional error is 14.9\%. The population amplitude is $\mu_A = 4.6 \pm 0.4$ with intrinsic scatter $\sigma_A \approx 1.5$; the mass‑scaling exponent $\gamma = 0.09 \pm 0.10$ is consistent with zero.

• Posterior ($\gamma$-free vs $\gamma=0$): $\Delta$WAIC $\approx +0.01 \pm 2.5$ (inconclusive).

• \textbf{Parsimony:} Given $\Delta$WAIC $\approx 0 \pm 2.5$, we adopt $\gamma=0$ as the preferred baseline (Occam's razor) and retain the mass‑scaled model as a constrained extension for future, larger samples.
...
Using a comprehensive validation suite, we performed a complete, parity-fair comparison between the TG-$\tau$ $\Sigma$-Gravity redshift prescription and a flat FRW cosmology with free intercept, employing the full Pantheon+ SH0ES dataset ($N = 1701$ SNe) and the official STAT + SYS compressed covariance.
...
\toprule
Model & $H_s$ / $\Omega_m$ & $\alpha_{\rm SB}$ / Intercept & $\chi^2$ & AIC & $\Delta$AIC & Akaike Weight \\
\midrule
TG-$\tau$ & $H_\Sigma = 72.00 \pm 0.26$ & $\alpha_{\rm SB} = 1.200 \pm 0.015$ & 871.83 & 875.83 & + 59.21 & 0.000 \\
FRW & $\Omega_m = 0.380 \pm 0.020$ & intercept $= -0.0731 \pm 0.0079$ & 812.62 & 816.62 & 0 & 1.000 \\
\bottomrule
\end{tabular}
\end{table}


\textbf{Fair-comparison outcome.} Both models were fitted with identical freedoms ($k = 2$). Under this parity, FRW remains the statistically preferred description ($\Delta$AIC $= + 59.21$ in its favor), but TG-$\tau$'s parameters are fully physical and stable:


\begin{itemize}
\item \textbf{$H_\Sigma = 72.00$ km s$^{-1}$ Mpc$^{-1}$} — consistent with $H_0 \approx 70$
\item \textbf{$\alpha_{\rm SB} = 1.200 \pm 0.015$} — intermediate between energy-loss (1) and Tolman (4) scaling
\item \textbf{$\xi = 4.8 \times 10^{-5}$} — matching the expected $\Sigma$-Gravity micro-loss constant
\end{itemize}
...
All validation items were executed and passed:
...
Test & Result & Pass \\
\midrule
Full covariance $\chi^2$ (Cholesky) & Consistent; stable fit & ✓ \\
Zero-point handling (anchored vs free intercept) & $H$ shift 8 km/s/Mpc; $\alpha_{\rm SB}$ stable & ✓ \\
$\alpha_{\rm SB}$ robustness (across $z$ slices) & $\alpha_{\rm SB} = 1.200$ for all bins & ✓ \\
Hubble residual systematics & no significant correlations & ✓ \\
ISW / hemispherical anisotropy & $\Delta\mu \approx 0.056$ mag (NS); $p > 0.05$ & ✓ \\
Bootstrap $\Delta$AIC stability & Stable under 1000 resamples & ✓ \\
Distance-duality diagnostic & $\eta(z) = (1+z)^{0.2}$ suggested & ✓ \\
\bottomrule
\end{tabular}
\end{table}
...
Hence $\eta(1) = 1.1487$ and $\eta(2) = 1.2457$; these values provide a clear, testable signature for future BAO or cluster $D_A$ datasets (see Fig. below).


(1$\sigma$ band from finite-difference Hessian.)


\textbf{Zero-Point Anchoring and Anisotropy}


Anchored fits ($H_\Sigma = 72.0$) and free-intercept fits ($H_\Sigma = 80.0$) yield identical $\alpha_{\rm SB} = 1.200$, confirming that absolute magnitude degeneracy does not impact the surface-brightness scaling. A full-sky dipole fit framework was implemented and validated with RA/DEC data — no significant directional bias detected ($p > 0.05$).
...
\begin{enumerate}
\item \textbf{Baryons:} \texttt{core/gnfw\_gas\_profiles.py}, \texttt{core/build\_cluster\_baryons.py} ($f_{\rm gas}=0.11$, clumping fix), \texttt{data/clusters/*.json}; per‑cluster $\Sigma_{\rm baryon}(R)$ CSVs ingested when available (A2261, MACSJ1149 hold‑outs).


\item \textbf{Triaxial projection:} \texttt{core/triaxial\_lensing.py} (global normalization; geometry validated).


\item \textbf{Projected kernel:} \texttt{core/kernel2d\_sigma.py} ($K_\Sigma(R)=A_c \cdot C(R;\ell_0,\cdots)$).


\item \textbf{Diagnostics (MACS0416):} \texttt{python scripts/plot\_macs0416\_diagnostics.py}

Generates: \texttt{convergence\_profiles.png}, \texttt{cumulative\_mass.png}, \texttt{convergence\_maps\_2d.png}, \texttt{boost\_profile.png}.
\end{enumerate}
...
Self-contained figures for convergence and deflection, calibrated to the observed $\theta_E$ in the catalog and using the paper's $\Sigma$-kernel $K(R)=A_c \cdot C(R;\ell_0,\cdots)$:
