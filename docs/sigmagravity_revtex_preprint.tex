...
\textbf{Implementation:} Since $\mathcal{C}$ depends on $v_{\rm rot}$ (which depends on $\Sigma$), the prediction requires fixed-point iteration:
\begin{enumerate}
...
where:
\begin{itemize}
\item $A_0 = e^{1/(2\pi)} \approx 1.173$ (disk galaxies)
\item $L_0 = 0.40$ kpc (calibrated scale)
\item $n = 0.27$ (path-length exponent)
\item $D = 0$ for disk-dominated, $D = 1$ for dispersion-dominated
\end{itemize}

For disk galaxies ($D=0$): $A = 1.173$. For clusters ($D=1$, $L \approx 600$ kpc): $A \approx 8.45$.

This unifies the galaxy and cluster regimes through a single principled relationship based on system dimensionality and path length through baryons (Fig.~\ref{fig:amplitude}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{amplitude_comparison.png}
\caption{Amplitude versus path length through baryons for 163 SPARC disk galaxies (green), 873 MaNGA ellipticals (orange), and 44 Fox et al. clusters (red). Blue line: $\Sigma$-Gravity prediction $A = A_0(L/L_0)^{0.27}$. Red dashed: MOND (scale-independent, $A \approx 1$). Gray dotted: GR without dark matter ($A = 0$). MOND works for galaxies but fails for clusters by $\sim$10$\times$. $\Sigma$-Gravity's path-length-dependent amplitude unifies all system types from sub-kpc disks to Mpc-scale clusters.}
\label{fig:amplitude}
\end{figure}
...
At Saturn's orbit ($r \approx 9.5$ AU), $g_N \approx 6.4 \times 10^{-4}$ m/s$^2$, giving $h(g_N) \approx 4 \times 10^{-4}$. Combined with $\mathcal{C} \ll 1$ for the Solar System, the total enhancement is $\Sigma - 1 < 10^{-8}$.

This implies $|\gamma - 1| \sim 10^{-9}$, well within the Cassini bound of $|\gamma - 1| < 2.3 \times 10^{-5}$ \cite{Bertotti2003} (Fig.~\ref{fig:solar}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{solar_system_safety.png}
\caption{Enhancement ($\Sigma - 1$) as a function of distance from the Sun. Blue line: $\Sigma$-Gravity prediction. Red/orange dashed: observational bounds (Cassini PPN, planetary ephemeris). The predicted enhancement is $< 10^{-14}$ throughout the Solar System---including at Voyager 1 (160 AU)---far below any detection threshold. The coherence mechanism automatically suppresses modification in compact, high-acceleration systems.}
\label{fig:solar}
\end{figure}
...
\item Apply enhancement: $\Sigma = 1 + A_0 \cdot \mathcal{C}(r) \cdot h(g_N)$
...
We use 42 strong-lensing clusters from Fox et al. (2022) \cite{Fox2022} with Einstein radii and spectroscopic redshifts. Lensing masses are derived from the critical surface density at the Einstein radius.
...
$\Sigma$-Gravity achieves near-unity median ratio with all 42 clusters within factor 1.5 of observations (Fig.~\ref{fig:clusters}). MOND systematically underpredicts cluster lensing masses by factor $\sim$3, requiring additional mass (often attributed to residual dark matter or massive neutrinos).

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{cluster_fox2022_validation.png}
\caption{$\Sigma$-Gravity cluster validation using 42 Fox et al. (2022) strong-lensing clusters. Left: Predicted vs. observed mass at 200 kpc aperture (1:1 line shown). Middle: Ratio vs. redshift showing no systematic evolution. Right: Distribution of $\log(M_\Sigma/M_{\rm SL})$ centered at zero with scatter = 0.132 dex. Unlike MOND (which underpredicts by $\sim$3$\times$), $\Sigma$-Gravity matches cluster lensing without additional parameters.}
\label{fig:clusters}
\end{figure*}
...
\begin{itemize}
\item Galaxy rotation curves (RMS $\sim$18 km/s)
\item Milky Way stellar velocities (RMS $\sim$30 km/s)
\item Cluster lensing masses (median ratio 0.99)
...
\caption{Counter-rotation test using MaNGA DynPop data. (A) Theory predictions vs. observation: $\Lambda$CDM/MOND predict no difference between counter-rotating and normal galaxies (ratio = 1.0); $\Sigma$-Gravity predicts reduced enhancement (ratio $< 1.0$, exact value depends on counter-rotating fraction). The observed ratio is $0.56 \pm 0.09$, consistent with $\Sigma$-Gravity and inconsistent with $\Lambda$CDM/MOND. (B) $f_{\rm DM}$ distributions: counter-rotating galaxies (red, N=63) show systematically lower inferred dark matter fractions than normal galaxies (gray, N=10,038). Mann-Whitney $p = 0.004$.}
\label{fig:counter}
\end{figure}
...
\textit{Observational status:} KMOS$^{\rm 3D}$ observations of $z \sim 1$--2 galaxies show reduced dark matter fractions compared to local galaxies, consistent with this prediction \cite{KMOS3D2020}.
...
\bibitem{MaNGA2023}
MaNGA DynPop Collaboration (2023), private communication.

\bibitem{KMOS3D2020}
KMOS$^{\rm 3D}$ Collaboration, Astrophys. J. (2020).
