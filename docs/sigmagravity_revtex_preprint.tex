...
\item \textbf{Satellite galaxies (dSphs):} Inherit $\Sigma$ from host galaxy at orbital radius (see SI \S7a)
\end{itemize}
...
\textbf{Baryonic mass estimate:} $M_{\rm bar}(200~{\rm kpc}) = 0.4 \times f_{\rm baryon} \times M_{500}$, where $f_{\rm baryon} = 0.15$ (cosmic baryon fraction). The factor 0.4 accounts for baryon concentration within 200 kpc (typical for NFW profiles with $c \sim 4$--6). Sensitivity analysis (SI \S6) shows that varying this factor by $\pm$25\% shifts the median ratio by $\sim\pm$30\%, within the observational scatter. The $M_{500}$ values are from X-ray/SZ observations, independent of lensing.
...
\label{eq:lens_dyn}
\end{equation}
For $\Sigma = 1$: $M_{\rm lens}/M_{\rm dyn} = 1$ (GR limit). For $\Sigma = 2$: $M_{\rm lens}/M_{\rm dyn} \approx 0.875$. This effective lensing closure is adopted as a phenomenological ansatz; a rigorous derivation from an action principle is deferred to future work. \textbf{The cluster lensing results are conditional on this prescription}; sensitivity analysis (SI \S11) shows the qualitative result is robust but the calibrated parameters would shift under alternative slip assumptions.
...
Dwarf spheroidal galaxies (dSphs) orbiting the Milky Way are dispersion-dominated with negligible internal coherent rotation. Rather than generating their own enhancement, they inherit the $\Sigma$-enhancement of their host galaxy at their orbital radius.

\textit{Observational status:} Preliminary analysis of classical dSphs (Fornax, Sculptor, Draco, Carina, Ursa Minor) shows median predicted/observed velocity dispersion ratio of $0.87 \pm 0.15$, consistent with the host inheritance model (see SI \S7a).
...
The data and code supporting this study are openly available at \url{https://github.com/lrspeiser/SigmaGravity}. The master regression script \texttt{scripts/run\_regression\_extended.py} reproduces all numerical results (17 tests) using the canonical parameters defined in SI \S2. The repository includes SPARC rotation curve analysis, cluster lensing predictions, Milky Way validation, and figure generation scripts.
