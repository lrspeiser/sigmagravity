...
$L_0$ & 0.40 kpc & Calibrated & Galaxy-cluster transition \\
...
\hline
\end{tabular}
\caption{Parameter accounting. $L_0$ and $n$ were calibrated using 42 Fox et al. clusters; SPARC galaxies provide independent validation.}
\label{tab:params}
\end{table}

$D = 0$ for disk-dominated systems, $D = 1$ for dispersion-dominated. For disk galaxies ($D=0$): $A = 1.173$. For clusters ($D=1$, $L \approx 600$ kpc): $A \approx 8.45$ (Fig.~\ref{fig:amplitude}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{amplitude_comparison.png}
\caption{Amplitude versus path length for 163 SPARC disk galaxies (green), 873 MaNGA ellipticals (orange), and 42 Fox et al. clusters (red). Blue: $\Sigma$-Gravity prediction. Red dashed: MOND. Gray: GR without dark matter.}
\label{fig:amplitude}
\end{figure}
...
\textbf{Lensing assumption:} We assume the enhanced potential $\Phi$ governs both dynamics and light deflection ($\Phi = \Psi$ in the weak-field limit). A rigorous relativistic derivation is deferred to future work.
...
\bibitem{Milgrom2010}
M. Milgrom, ``Quasi-linear formulation of MOND,'' Mon. Not. R. Astron. Soc. \textbf{403}, 886 (2010).
