#!/usr/bin/env python3
"""
Evaluate V2.3 B/T Laws on SPARC Sample
======================================

Evaluates V2.3 B/T laws with radius-dependent bar/shear suppression:
- Bar taper: suppress ring beyond R_bar in SB/SAB galaxies
- Shear taper: suppress ring beyond R_shear in high-shear disks
- Shear-coupled sigma_ring: narrow envelope in high-shear disks
- Dynamic M_max(S, B/T): prevent red roofs

Usage:
    python many_path_model/bt_law/evaluate_bt_laws_v2p3.py
"""
import argparse
import json
import sys
...
from bt_laws_v2p3 import (load_theta, eval_all_laws_v2p3, ring_radial_envelope,
                          bar_radial_taper, shear_radial_taper)


def compute_galaxy_ape_v2p3(galaxy: SPARCGalaxy, params: dict) -> float:
    """
    Compute APE using V2.3 many-path model with radius-dependent suppression.
    
    Args:
        galaxy: SPARC galaxy data
        params: Parameter dict with eta, ring_amp, M_max, lambda_ring, kappa, 
                R_bar, w_bar, gamma_bar_taper, R_shear, w_shear, gamma_shear_taper, etc.
    ...
    # Many-path multiplier M(r) with V2.3 enhancements
    eta = params['eta']
    ...
    # Ring concentration (from V2.1/V2.2)
    R_ring = params.get('R_ring', 8.0)
    sigma_ring = params.get('sigma_ring', 0.5)  # Now shear-dependent in V2.3
    ...
    # Radial concentration envelope (Gaussian)
    ...
    # V2.3 NEW: Radius-dependent bar taper (SB/SAB only)
    ...
    # V2.3 NEW: Radius-dependent shear taper (high-shear only)
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate V2.3 B/T Laws on SPARC')
    parser.add_argument('--bt_params', type=Path,
                       default=Path('many_path_model/bt_law/bt_law_params_v2p3_initial.json'),
                       help='Path to v2.3 B/T law parameters')
    ...
    parser.add_argument('--output_dir', type=Path,
                       default=Path('results/bt_law_evaluation_v2p3'),
    ...
    # Load parameters
    print("="*80)
    print("V2.3 B/T LAW EVALUATION ON FULL SPARC SAMPLE")
    print("  (With Radius-Dependent Bar/Shear Suppression)")
    print("="*80)
    print(f"\nLoading v2.3 B/T law parameters from: {args.bt_params}")
    ...
    for param_name, param_val in theta.items():
        ...
        elif param_name.startswith('comment'):
    ...
    print("\n" + "="*80)
    print("EVALUATING GALAXIES WITH V2.3 LAWS (RADIUS-DEPENDENT TAPERS)")
    print("="*80)
    ...
    for i, rotmod_file in enumerate(rotmod_files, 1):
        ...
        try:
            ...
            # Predict parameters using v2.3 laws with radial tapers
            v2p3_params = eval_all_laws_v2p3(B_T, theta,
                                             Sigma0=Sigma0,
                                             R_d=R_d,
                                             shear=shear,
                                             g_bar_in=g_bar,
            ...
            # Compute APE with v2.3 parameters (including radial tapers)
            ...
            result = {
                ...
                'delta_ape': v2p3_ape - per_gal_ape if per_gal_ape else None,
                'v2p3_params': v2p3_params,
            ...
            status += f" λ={v2p3_params['lambda_ring']:>5.1f}"
            status += f" σ={v2p3_params['sigma_ring']:.2f}"
            
            # Show taper info
            if v2p3_params.get('R_bar'):
                status += f" R̄={v2p3_params['R_bar']:.1f}"
            if v2p3_params.get('R_shear'):
                status += f" R_s={v2p3_params['R_shear']:.1f}"
            
            status += f" | V2.3: {v2p3_ape:6.2f}%"
            if per_gal_ape:
                status += f" | PG: {per_gal_ape:6.2f}% | Δ: {v2p3_ape - per_gal_ape:+6.2f}%"
            print(status)
    ...
    # V2.3 Law statistics
    v2p3_apes = [r['v2p3_ape'] for r in results]
    print("\n" + "-"*80)
    print("V2.3 B/T LAW PERFORMANCE")
    print("  (Radius-Dependent Bar/Shear Suppression + Shear-Coupled σ + Dynamic M_max)")
    print("-"*80)
    print(f"  Mean APE:       {np.mean(v2p3_apes):.2f}%")
    print(f"  Median APE:     {np.median(v2p3_apes):.2f}%")
    print(f"  Std Dev:        {np.std(v2p3_apes):.2f}%")
    print(f"  Min APE:        {np.min(v2p3_apes):.2f}%")
    print(f"  Max APE:        {np.max(v2p3_apes):.2f}%")
    
    # Quality distribution
    excellent = sum(1 for ape in v2p3_apes if ape < 10)
    good = sum(1 for ape in v2p3_apes if 10 <= ape < 20)
    fair = sum(1 for ape in v2p3_apes if 20 <= ape < 30)
    poor = sum(1 for ape in v2p3_apes if ape >= 30)
    
    print(f"\nQuality Distribution:")
    print(f"  Excellent (< 10%):   {excellent:3d} ({100*excellent/len(v2p3_apes):.1f}%)")
    print(f"  Good (10-20%):       {good:3d} ({100*good/len(v2p3_apes):.1f}%)")
    print(f"  Fair (20-30%):       {fair:3d} ({100*fair/len(v2p3_apes):.1f}%)")
    print(f"  Poor (≥ 30%):        {poor:3d} ({100*poor/len(v2p3_apes):.1f}%)")
    ...
    if valid_comparisons:
        ...
        print(f"\n  Mean Δ (V2.3 - PerGal):  {np.mean(deltas):+.2f}%")
    ...
    print("\n" + "-"*80)
    print("PERFORMANCE BY BAR CLASS (V2.3 Law)")
    print("-"*80)
    for bar_cls in ['SA', 'S', 'SAB', 'SB']:
        bar_results = [r for r in results if r['bar_class'] == bar_cls]
        if bar_results:
            bar_apes = [r['v2p3_ape'] for r in bar_results]
    ...
    # Count galaxies with radial tapers applied
    bar_taper_count = sum(1 for r in results if r['v2p3_params'].get('R_bar') is not None)
    shear_taper_count = sum(1 for r in results if r['v2p3_params'].get('R_shear') is not None)
    
    print("\n" + "-"*80)
    print("RADIAL TAPER STATISTICS")
    print("-"*80)
    print(f"  Bar taper applied:    {bar_taper_count:3d} galaxies (SB/SAB)")
    print(f"  Shear taper applied:  {shear_taper_count:3d} galaxies (high-shear)")
    
    # Save results
    args.output_dir.mkdir(parents=True, exist_ok=True)
    
    # Full results JSON
    output_file = args.output_dir / 'v2p3_evaluation_results.json'
    output_data = {
        'bt_law_version': 'v2.3_radius_dependent_suppression',
        'bt_law_params': theta,
        'evaluation_time': elapsed,
        'total_galaxies': len(rotmod_files),
        'successful': len(results),
        'bar_taper_count': bar_taper_count,
        'shear_taper_count': shear_taper_count,
        'v2p3_statistics': {
            'mean_ape': float(np.mean(v2p3_apes)),
            'median_ape': float(np.median(v2p3_apes)),
            'std_ape': float(np.std(v2p3_apes)),
            'min_ape': float(np.min(v2p3_apes)),
            'max_ape': float(np.max(v2p3_apes))
        },
        'results': results
    }
    ...
    # Summary CSV
    csv_file = args.output_dir / 'v2p3_evaluation_summary.csv'
